...
## [23.0] - 2026-09-09 · **אדמין 5.4 — תשאול בונה את החודש**

מספר האפליקציה (23.0) ומספר המוח (3.0) לא זזו. פריט 7 של המזמין.

### 7 · הכפתור "השלם את החודש עם המאמן" בוטל
מילוי תשאול במודול האדמין **הוא** ההוראה לבנות את החודש, ולכן שני הרגעים
שמייצרים חודש ריק מתחילים את הבנייה בעצמם:

- **לקוח שנוצר מתשאול** — הלבנה הראשונה נכתבת מיד.
- **"צור לבנה חדשה"** — נשאר הדרך לחודש הבא, ועכשיו גם כותב אותו.

מה שלא זז: הבנייה רק מפעולה שלו, לעולם לא בטיימר, לא על חודש שכבר נכתב,
ולא אצל לקוח ריק. מה שנשלח למאמן נשאר קריא — מקופל תחת שורת ההתקדמות,
במקום מסך אישור שעוצר אותו — והחודש נוחת **לא מאושר**.

### בונוס: הלבנה הבאה יודעת מה הקודמת עשתה
בקשת לבנה שממשיכה תוכנית נושאת את סיכום החודש שלפניה (`blockHandoff`) — המשקלות
שזזו, הפורמטים ששימשו והבקשות הקבועות — כפי שהמוח ביקש.

### בדיקות
4,580 בדיקות ירוקות, מהן 27 חדשות לפריט הזה — כולל הרצה אמיתית של ההחלטה
לבנות ושל בונה המסירה, לא רק קריאת קוד.

## [23.0] - 2026-09-09 · **אדמין 5.3 — עריכה מקבילה, "היום", וימים שלא קיימים**

מספר האפליקציה (23.0) ומספר המוח (3.0) לא זזו. הכל במודול האדמין, לפי המספור של המזמין.

### 5 · מצב עריכה מקבילה
עד עכשיו המערכת החזיקה **טיוטה אחת**: פתיחת יום שני שמרה וסגרה את הראשון. עכשיו כל יום
פתוח מחזיק טיוטה משלו, וכל פקד בכרטיס יודע לאיזה יום הוא שייך — הכפתורים, ENTER בשורת
עבודה ובהערה, הצבע, המספור, ה-×, כותרת היום, הצ'קבוקס "יום מנוחה", ו-Save/Cancel לכל
כרטיס בנפרד.

- **גרירה בתוך היום** משנה סדר, עם ההנפשה של הלשוניות והמספור שמתעדכן מיד.
- **גרירה לכרטיס יום אחר שפתוח** מעתיקה את החלק לשם ומשאירה אותו במקור (5.א); יום שעוד
  לא פתוח לעריכה נפתח בשבילך.
- **מה נכתב ומתי:** כלום עד Save על אותו כרטיס, או עד שיום יוצא מהמסך. בחירת ימים כותבת
  רק את מה שכבר לא נראה; החלפת שבוע/תצוגה/"היום" או מעבר ללקוח אחר כותבת את כל מה שפתוח.
- תצוגת הלקוח בלינק נשארה עם יום אחד פתוח, בכוונה, וכך גם העורך הישן.

### 6 · "היום" בלוח השנה של ניהול כללי
כפתור באדום בהיר שמחזיר לחודש הנוכחי; אם אתה כבר בו — ריבוע היום מהבהב במקום טעינה
מחדש. היום הנוכחי מוקף במסגרת, בנפרד מהמסגרת שאומרת "היום הזה פתוח".

### 8 · ימי מנוחה שנשתלו בסימון כל הלבנה
בחירה היא חשבון על גריד של שבעה ימים, ולכן בתוכנית של שלושה אימונים בשבוע סימון של כל
ימי הלבנה ביקש ארבעה כרטיסים בשבוע שלא נמכרו — והם הוצגו כימי מנוחה עם תאריכי יומן.
עכשיו יש פילטר אחד לכל דרכי הבחירה, שמכבד גם שבוע שקוצץ בנפרד, והתצוגה מסרבת לצייר
משבצת כזו גם אם יבקשו ממנה.

### בדיקות
4,546 בדיקות ירוקות, מהן 46 חדשות לסבב הזה, בתוספת בדיקה בדפדפן אמיתי שבה כל פקד
בכרטיס השני מדווח על היום השני.

## [23.0] - 2026-09-09 · **אדמין 5.2 — חלקי האימון, והיום שנתקע**

מספר האפליקציה (23.0) ומספר המוח (3.0) לא זזו. כל השינויים במודול האדמין ובעמוד
שהלקוח פותח בלינק, לפי המספור של המזמין.

### 1.א · שינוי שם חלק = שינוי שם מלא
"PART A" לא היה שם אלא תוספת שהתצוגה הדביקה לכל כותרת, ולכן חזר גם אחרי שינוי שם.
עכשיו הכותרת היא בדיוק השם שנכתב, ושדה העריכה מכיל רק אותו. גם כותרות שהמוח כתב
("Part B - bench press Progression") מוצגות נקיות, בלי לשנות שום נתון שמור.

### 1.ב · מספור אוטומטי לחלקים
המספר הוא המקום של החלק — 1 בראש ומטה — ולכן הזזת חלק ממספרת מחדש את כולם מעצמה.
המספר מצויר בעיגול, ופותח את הכותרת בצד לפי כיוון הדף: מימין באדמין, משמאל בעמוד
הלקוח. השם עצמו ממשיך להסתדר לפי השפה שלו.

### 1.ג · שינוי סדר חלקים בגרירה
אחיזה בידית וגרירה למעלה או למטה בתוך אותו יום, עם קו שמסמן לאן זה נופל ועם ההנפשה
של הלשוניות. גרירה בין שני ימים פתוחים ממשיכה לעבוד כשהייתה.

### 2 · ENTER בשורת NOTE
פותח NOTE חדש מתחת עם הסמן בפנים, בדיוק כמו בשורת WORK LINE.

### 3 · יום מנוחה שנשתל ולא היה אפשר להסיר (עדיפות א')
הכלל שקובע "יום מנוחה" קרא קודם את שורת הסקירה השבועית ורק אחר כך את מה שכתוב ביום,
וכרטיס מנוחה מסתיר את החלקים — כך שאימון שנכתב ביום שסקירתו אמרה Rest נשאר שמור אבל
בלי דרך להגיע אליו, למחוק או להחליף. עכשיו **אימון כתוב גובר על שורת סקירה**, כמו
שהמודול של יום המנוחה קרא את זה מאז ומתמיד — ולכן יום במצב הזה מסתדר לבד ברענון.
בנוסף: הצ'קבוקס "יום מנוחה" שואל לפני שהוא מסמן על משהו שהוקלד, ושמירה אוטומטית ביציאה
מהיום לא הופכת עבודה שהוקלדה ליום מנוחה.

### 4 · תת־הכותרת בהשוואת אימונים בין שבועות
התצוגה קיבלה כותרת אחת לכל הכרטיסים, לפי היום שנבחר אחרון — ולכן ארבעה כרטיסים אמרו
"אימון 1 · שבוע 4". כל כרטיס נשאל עכשיו בנפרד, ונושא את השבוע והמשבצת שלו.

### בדיקות
4,480 בדיקות ירוקות, מהן 82 חדשות לסבב הזה, בתוספת רנדור בדפדפן אמיתי של הכרטיס
ושל העורך.

## [23.0] - 2026-09-08 · **אדמין 5.1.1 — הכפתור של המאמן עובד**

מספר האפליקציה (23.0) ומספר המוח (3.0) לא זזו. תיקון במודול האדמין בלבד.

### מה היה
לחיצה על "המאמן יכתוב את החודש" אצל לקוח סטודיו אמיתי החזירה פאנל אדום:
"בונה התשאול של הסטודיו לא נטען". התשאול עצמו היה תקין לגמרי — הבאג היה בקוד:
ספריית התשאול מתפרסמת בדפדפן בשם `CLIENT_INTAKE`, והקובץ החדש חיפש שם אחר,
ובנוסף חיפש אותה ברגע שהוא עצמו נטען — לפני שהיא בכלל נטענה בדף.

### מה תוקן
- הספריות נקראות ברגע הלחיצה, ובשם הנכון.
- `lib/client-intake.js` נטען עכשיו לפני הקבצים שקוראים אותו.
- בדיקה חדשה מדמה דפדפן אמיתי (סדר טעינה הפוך) כדי שזה לא יחזור.

## [23.0] - 2026-09-08 · **אדמין 5.1 — המאמן כותב את החודש**

מספר האפליקציה לא זז (23.0) ומספר המוח לא זז (3.0). מה שעלה הוא **מודול האדמין**.

- **כפתור בלשונית של לקוח: "תן למאמן לכתוב את החודש"** (סטודיו או אדם אחד; לקוח ריק נדחה — הוא בשבילך ידנית).
- **לפני שיוצא שקל** — מסך שמראה את **הטקסט המדויק שנשלח למוח**: התשאול שלך כפי שהוא שמור, ולידו סוג הלקוח, איזו לבנה, איזה שבוע דילואד, הזמן והעלות. אתה מאשר, ורק אז זה יוצא.
- **חודש שלם, ארבע קריאות בטור:** שבוע 1 מהמוח ואז מילוי של 2, 3, 4 — כל שבוע נשמר ומצויר כשהוא נוחת, עם "בונה שבוע 3 מתוך 4" והמשכה מנקודת כשל. קריאה בודדת מחזירה שבוע אחד ושלוש כותרות, ולכן זה מה שהופך אותה לחודש.
- **החודש נכתב לא מאושר** — הלקוח לא רואה כלום עד שאתה שולח.
- **הכלל שלא זז:** באפליקציה אין AI, ובדף שהלקוח פותח בלינק אין AI. הכל בתוך המודול שלך בלבד, ורק בלחיצה שלך.

## [23.0] - 2026-09-08 · **אדמין 5.0 · מאמן 3.0**

גרסת האפליקציה לא זזה — 23.0 כמו שהיא. מה שעלה הוא **מודול האדמין** והמספר של **מוח המאמן**.

### לשוניות ולוח האימונים
- **שינוי סדר הלשוניות בגרירה** — בעכבר, ובנייד בלחיצה ארוכה. "ניהול כללי" נשאר ראשון. הסדר נשמר בשרת ועובר בין המחשב לטלפון.
- **העתקת חלק אימון** — מקש ימני בכל נקודה על יום פתוח, או גרירה מהידית בין ימים שעל המסך. החלק נשתל מתחת לאחרון ולא דורס כלום.
- **העתקה בין לקוחות** — שבוע, יום או חלק עוברים מלקוח ללקוח ושורדים רענון דף.
- **מחסן לבנות** — לבנה שמורה על המדף עם שם, תיאור וסוג, ונשתלת אצל כל לקוח בלחיצה.
- **תפריט לבנה מלא** — העתק, מועדפים, הדבקה לפני, הכפלה, מחיקה. וגרירת לבנות לשינוי סדר.
- **שם ללבנה** — "Block 1 · Sep 6 – Oct 3 2026 · כוח בסיסי". השם נוסע עם הלבנה והלקוח רואה אותו.
- **כמות אימונים בשבוע — מנקודת זמן והלאה** (ללקוח ריק בלבד). שבועות קודמים לא זזים, ושום דבר לא נמחק — אימון שיוצא מהתצוגה חוזר אם מחזירים את המספר.
- **מחיקת חלק שלם** ב-× אחיד, ומספור שהפכנו לגלולה עם תיבת סימון אמיתית.
- **החלקה בין ימים בטלפון** בצד הלקוח.

### ניהול כללי
- **"סכום לחשבונית קרובה"** — כל מה שלא הוצאה עליו חשבונית, מאז ומעולם, בלי קשר לטווח או לסינון. נשמר כמספר אחד בשרת שנכתב מחדש רק כשחודש משתנה.

### מוח המאמן 3.0
- המוח החדש חי בפרודקשן — חמש-עשרה שכבות ידע מנותבות במקום שני קבצים סטטיים.
- **הצד שלנו מוכן ולא מחובר:** שני השדות שהמוח ביקש (`deloadWeekIndex`, `blockHandoff`), תצוגת ההערות שלו, ותזמור של ארבע הקריאות שכותב חודש שלם — הכל בנוי ונבדק.
- **ולא מופעל במכוון:** בעל המוצר לוקח על עצמו את מוטת השליטה המלאה על יצירת הלבנות, ולאף אחד אין דרך לשנות אימונים על בסיס AI.

### באגים שנסגרו
- הדף נתקע על "טוען" — המודול נפתח לפני שהדף סיים להיטען; עכשיו הוא ממתין, ואם באמת נכשל — אומר זאת.
- חלק שנתתה לו כותרת בלי שורות הוצג כ"Full session details still loading…". כותרת שכתבת היא תוכן.
- הקובייה הריקה של העורך נשמרה לפני מה שהדבקת — לא עוד.
- גרירת לבנה וגרירת חלק לא עבדו בכלל — היו מחוברות לאלמנט שלא קיים בזמן הטעינה.
- פריסת הלוח לא התכווצה אחרי צמצום אימונים — כל חודש נמדד עכשיו לפי עצמו.
- טבלת המחסן נעלמה אחרי רענון — הבקשה יצאה לפני שהסשן היה מוכן.

## [23.0] - 2026-09-08 · **מח המאמן החדש — מחובר**

עד היום `/api/personal-coach` תכנן משני קבצים סטטיים של 6,884 תווים. מעכשיו הוא מתוכנת על ידי **חמש-עשרה שכבות ידע מנותבות**, שכל אחת אושרה שורה-שורה, ושנבחרות לפי מי המתאמן: המוצר, ההגבלות, המיקוד שהוצהר, והאם זו לבנה ראשונה או המשך. חבילה בפועל: 24,679 תווים לסטודיו, 28,287 למתאמן רגיל, עד 50,000 למקרה הכבד.

### מה שהבדיקות מצאו — ואף אחד מזה לא היה נמצא בקריאת קוד

ארבעה תרחישים אמיתיים רצו דרך הנתיב החי, בלי לשמור דבר: מתאמן קצה (4 שבועות), סטודיו נשים (2), ספורטאית מתחרה (לבנה שלמה + לבנת המשך), ובוקס שכונתי (3). 24 קריאות, ₪2.28.

- **חמישה-עשר מקומות בשרת ביקשו לבנה של חמישה שבועות** בזמן שהחבילה ביקשה ארבעה — שתי הוראות סותרות באותה בקשה, וההוראה של השרת הייתה החזקה. הדילואד הפך מהנחה לקלט.
- **כלל ההתחלה באמצע שבוע גנב שני ימי אימון בכל שבוע אחרי הראשון.** מוגבל עכשיו לשבוע שמכיל את היום.
- **סטודיו שקנה ארבעה אימונים קיבל אחד.** לחדר שנמכר לפי מספר אימונים אין לוח שנה לקצץ.
- **שבוע 2 של לבנה לא ראה את שבוע 1 בכלל** ושכפל את יום שבת תנועה בתנועה. נבנתה מסירה תוך-לבנתית: מלאי תנועות ופורמטים לפי יום, מקוצץ, בלי משקלים.
- **אחוזי 1RM נכתבו למי שלא מסר אף הרמה.** הפך לעובדה מחושבת על הבקשה.
- **תקרת הפלט 8,192 קטעה את הלבנה הגדולה ביותר באמצע JSON.** המודל סופר את החשיבה שלו מול אותו תקציב. הועלתה ל-32,768.
- **באג עברית בנתב שלל שכבות שלמות:** שמונה מתשעה ניסוחים טבעיים ("לשפר את הסנאץ שלי") לא הדליקו כלום — תחיליות, `\w` שלא מכיר עברית, ושלושה סוגי גרש.

### כללים חדשים, אחרי הכרעות בעל המוצר

- **POL-029 — הלקוח נשאר ומשתפר.** שתי לבנות זהות הן כשל בפני עצמו, גם כששתיהן טובות. חל גם **בין שבועות בתוך לבנה** ולא רק בין לבנות. אצל סטודיו הדרישה חזקה יותר: לחדר אין מקסימום להזיז, ומי שבא לשיעור לא יכול לבקש רוויזיה.
- **שער מבחן 1RM.** מתאמן כללי פעם בשש לבנות, מתחרה פעם בארבע, הכי מוקדם בלבנה השלישית. מחושב ממספר הלבנה, לא נתון לשיקול המאמן. **מחייב את המאמן ולא את המאמן האנושי** — הוראה של בעל המוצר פותחת את החלון בכל רגע.
- **בסטודיו הפורמט הוא הגיוון** — רוטציה נפרדת לכל סוג משבצת, והחוגות (אורך חלון, יחס עבודה/מנוחה) הן ההתקדמות ולא הגיוון.
- **שני חלקי עבודה לאימון.** שעה חלקי שלושה היא שמונה-עשרה דקות לחלק לפני מעברים.
- **"לכלול חימום?"** — שדה תחקור בשני המוצרים. "בלי חימום" לעולם אינו "בלי פריימר".
- **התקן נכתב על השורה, ההתאמה מתחתיה.** Ring row הוא התאמה של מתח, לא מרשם. הדאמבל הוא 22.5 ק"ג ומשם יורדים.
- **מתאמן אחד מקבל מספר אחד** — סימון Rx/scaled נכתב לחדר.

### חדש בתגובה

- **`brickFlags`** — בדיקה דטרמיניסטית, בלי קריאה למודל ובלי עלות. עד שישה משפטים, **לא פוסלת ולא מנסה שוב.** מכסה רק מה שמכונה בטוחה בו.
- **`truncated`** — סמן BLOCK_JSON שנפתח ולא נסגר. קטיעה ולא סירוב, והתיקון לה הוא ניסיון חוזר.

### קורא, ועדיין לא נשלח

`blockStartWeek` ו-`blockHandoff` נקראים משני מקומות — גוף הבקשה או הפרופיל — ומודול אדמין עוד לא מעביר אותם. עד שיעביר: **שער ה-1RM לא ייפתח לאף אחד, ולבנת המשך תיבנה כאילו לא היה חודש קודם.** הלבנה הראשונה של כל לקוח מלאה ותקינה בלעדיהם.

### חוב מוכר, לא נגענו

התצוגה מרפדת לשבוע חמישי (`normalize-pprog-block.js`) · בונה התחקור מצד המתאמן ב-`index.html` עוד אומר חמישה שבועות, מאחורי מפסק כבוי · התפלגות זמני העבודה יושבת 83% בחלון 10-20 דקות ושלושת המסלולים המטבוליים בלי פריסה מוצהרת — **הוצג והוכרע לא לשנות עכשיו** · שתי בדיקות מיותמות נכשלות ואינן בשרשרת.

## [22.3.3] - 2026-09-05 · **HOTFIX: מספור ישר בקצה השמאלי בפלאפון**

- **קוסמטי בלבד, ורק בפלאפון:** בעיברית העיגול עם המספר נדד לאמצע השורה, במקום שונה בכל שורה. עכשיו העיגולים עומדים בטור ישר בקצה השמאלי של המסך, גם כשהכתב בעברית.
- **איך:** בפלאפון השורה מסודרת משמאל לימין, והטקסט שומר על הכיוון שלו בתוך תיבה משלו — כך ששורה בעברית עדיין נקראת מימין לשמאל ושורה באנגלית לא זזה מילימטר.
- **מה לא נגעתי בו:** שורה בלי מספור לא השתנתה בכלל — אותו קוד, אותה תצוגה. בדפדפן במחשב שום דבר לא זז: הכלל חל רק על מסך צר מ-720 פיקסלים.

## [22.3.2] - 2026-09-05 · **HOTFIX: חלק זוכר את הצורה שלו**

- **באג בפרודקשן:** עריכה של שורת אימון לא "נתפסה" — ההערה והכותרת השתנו, השורה לא. הסיבה איננה בשמירה אלא בציור: הכרטיס **ניחש מחדש בכל פעם** מה הערה, מה שורת פורמט ומה שורת אימון, והניחוש בנוי על מילים באנגלית ("note:", "AMRAP", "3 rounds"). באימון בעברית אין להן זכר, ולכן שורות זזו בין התיבות בכל ציור מחדש.
- **עכשיו חלק זוכר את הצורה שלו:** כמה שורות בראשו נכתבו כהערות, והאם השורה שאחריהן היא שורת הפורמט. הצורה **נקראת ולא מנוחשת**, כך שמה שכתבת הוא מה שתראה — בעריכה, אחרי שמירה, ואצל הלקוח בלינק.
- הצורה נוסעת גם עם העתקה (חלק/יום/שבוע) ועם מה שהלקוח מקבל, כך שעותק נקרא בדיוק כמו המקור.
- **לא נגענו בישן:** חלק שלא נכתב בעורך — כל מה שנוצר לפני, וכל מה שמוח המאמן בנה — מסווג בדיוק כמו קודם. הצורה נרשמת בפעם הראשונה שהיום נשמר מהעורך.

## [22.3.1] - 2026-09-05 · **HOTFIX: העתקה נאמנה למקור**

- **באג בפרודקשן:** העתקת יום (וגם שבוע) שתלה את כל השורות כשורות אימון רגילות — ההערות, שורת הפורמט, הצבעים והמספור נעלמו, וגם שם היום שנתת. הסיבה: ההעתקה בנתה כל חלק מחדש מכותרת ומשורות בלבד.
- **עכשיו העתקה היא העתקה.** חלק מועתק נושא איתו את כל מה שיש בו — כמה שורות נכתבו כהערות, הצבע של כל שורה, המספור והמספרים שנכתבו מעל הספירה — והיום נושא את השם שנתת לו. רק המזהה מתחדש, ודגלי "מי שינה את זה" לא מועתקים, כי עותק טרי עוד לא שינה אף אחד.
- חל על שני הצדדים: ההעתקה שלי במודול, וההעתקה שהלקוח עושה בדף שלו.
- נבדק על **חמישה סוגי תוכניות** — יום ושבוע, עד מה שמגיע ללקוח בפועל דרך הלינק.

## [22.3] - 2026-09-05 · **כותרת לאימון, צבע לשורה, מספור לחלק**

### כותרת לכל יום אימון
- **כותרת ניתנת לעריכה** לכל יום, בשני הצדדים ובכל סוגי הלקוחות. הכותרת האוטומטית — "אימון 3 · שבוע 4" — נשמרת כהערה קטנה בצהוב בהיר ונטוי ליד העיפרון, ובמצב עריכה ליד דגל ה-Editing. יום שלא נערך ממשיך להציג את הכותרת האוטומטית **וגם** את ההערה לצידה.
- **AUTOSAVE לכותרת.** שינית שם ועברת ליום אחר בלי ללחוץ Save — השם נשמר. גם על יום ריק לגמרי: השמירה סירבה קודם לכתוב יום בלי אימון, אז שם על יום ריק פשוט נזרק. עכשיו השם נשמר לבדו והיום נשאר ריק — לא אימון ולא יום מנוחה.
- שם שנתת ליום **שורד** הפיכה ליום מנוחה וחזרה.

### צבע ומספור לשורות האימון
- **צבע לשורת אימון:** במצב עריכה, עיפרון בקצה כל שורה פותח פלטה של שישה צבעים עדינים + "ללא צבע". השורה נצבעת מיד, עוד לפני השמירה.
- **"Add numbering"** — צ'קבוקס ליד כותרת המשנה (Part A), רק במצב עריכה. ממספר את שורות האימון של אותו חלק בלבד; חלק אחד יכול להיות ממוספר והשני לא. המספר מוצג כספרה בתוך עיגול **ולוקח את צבע השורה שלו**.
- **המספרים ניתנים לעריכה** — ספרות בלבד. ברירת המחדל היא הספירה 1, 2, 3; כתבת 1 מעל השני — הוא נשאר 1, השלישי ממשיך להיות 3, והשורה הבאה שתתווסף היא 4. נשמר רק מספר שנכתב מעל הספירה.
- **ENTER בתוך שורת אימון** פותח שורה חדשה מיד מתחתיה והסמן קופץ אליה. הצבע והמספור של השורות שמתחת יורדים איתן.
- הכל עובר בלינק ללקוח וחוזר. שם צבע שלא קיים או מספר שאינו מספר נזרקים בשער ולא מגיעים לכרטיס.

### עברית בכרטיס האימון
- כל שורה מתיישרת לפי **האות הראשונה שלה**: שורה שמתחילה באנגלית נראית בדיוק כמו היום, ושורה שמתחילה בעברית נקראית מימין לשמאל — "יד קדמית 3X15". שורה־שורה, כך שכרטיס אחד יכול להחזיק את שתיהן.

### באגים שנסגרו
- **הערה שהפכה לאימון:** NOTE שנכתבה בעברית חזרה אחרי שמירה כשורת אימון. חלק זוכר עכשיו כמה מהשורות שלו נכתבו כהערות, במקום לנחש לפי מילים באנגלית.
- **ה-× של הערה לא עבד בכלל** — לא במסך הלקוחות שלי ולא בדף הלקוח. הוא קרא לעורך אחר בדף ולא עשה כלום.
- **העיפרון של הצבע נראה מת:** הפלטה נפתחה כחלון צף ונחתכה על ידי רצועת הכרטיסים. היא נפתחת עכשיו בתוך השורה, בשורה משלה מתחתיה.
- **הרצועה העליונה אומרת שהיא מחכה:** שני חצאי הרצועה מגיעים בשתי בקשות, ובהתעוררות קרה של השרת הלקוחות יכולים לאחר בכמה שניות. השתיקה הזו נקראה פעמיים כ"כל הלקוחות נעלמו" — עכשיו מופיע צ'יפ "טוען לקוחות…" עד שהם נוחתים.

## [22.2.1] - 2026-09-05 · **התחברות שלא נתקעת**

- **באג:** מסך ההתחברות יכול היה לשבת על "מתחבר…" בלי סוף — בקשה שלא נענתה השאירה אותו שם בלי שום דבר ללחוץ עליו. עכשיו יש תקרה של 15 שניות: הבקשה נקטעת והמסך אומר **"השרת לא ענה תוך 15 שניות"**, וכפתור הכניסה חוזר לעבוד.
- אותו דבר לחיבור אוטומטי דרך "זכור אותי": אם השרת לא ענה, המסך אומר **"השרת לא ענה — התחבר שוב"** במקום לחזור לטופס ריק בלי סיבה גלויה.

## [22.2] - 2026-09-05 · **להעתיק יום, להעתיק שבוע — בשני הצדדים**

- **העתק/הדבק יום** בנוסף לשבוע: קליק ימני על יום בלוח, ובנייד **לחיצה ארוכה** (חצי שנייה). היום נושא איתו גם את שורת הפוקוס שלו — שם רשום "Rest" — כך שהעתקת יום מנוחה באמת הופכת את היעד ליום מנוחה.
- **גם בצד הלקוח.** הלקוחות שלנו הם מאמנים שבונים אצלנו תוכניות, אז אותה תנועה קיימת בדף שלהם — ונכתבת דרך **העריכה הרגילה של הלקוח**: אותו מערך `edits`, בשמירה אחת, כך ששבוע מועתק הוא גרסה אחת ודגל אחד למאמן ולא שבעה. אין פעולת שרת חדשה ואין הרשאה חדשה.
- **התפריט עוצב מחדש** — כרטיס שקט עם כותרת, אייקון לכל פעולה וירוק על ההדבקה, במקום כפתורים כתומים שנראו כמו אזהרה.
- **לקוח ריק: "סוג תוכנית"** — שבועית (ימים) או מספר אימונים בשבוע. בחרת 4 → כל שורה מציגה ארבעה עיגולים; לבנה של 6 שבועות → שש שורות כאלה. "כמות האימונים" נשאלת רק כשהיא התשובה, כסיעוף של הבחירה.
- **צבע לכל סוג לקוח** בחלון "לקוח חדש": תכלת לאדם אחד, סגול לסטודיו, טורקיז ללקוח ריק.
- **באג שנסגר:** הכלל שאומר "מה שמוסתר — מוסתר" היה מקוצץ בטעות ל-`#clientScreen [hidden]`, כך שמחוץ למסך הלקוחות כל עיצוב שלנו גבר עליו. הוחזר לכלל גלובלי.
- העתקת יום ושבוע נבדקה על **חמישה סוגי תוכניות** — סטודיו שבועי, סטודיו במספר אימונים, אינדיבידואל, לקוח ריק שבועי ולקוח ריק במספר אימונים — כולל מה שמגיע ללקוח בפועל דרך הלינק.

## [22.1.1] - 2026-09-04 · **מסך אחד בכל רגע**

- **באג:** יצירת לקוח חדש השאירה את לשונית הניהול על המסך לצידו — שניהם ילדים של אותה שורה, אז הם חילקו את הרוחב וקיבלת חצי מסך מכל אחד. עכשיו, כשמסך הלקוח תופס את העמוד, לשונית הניהול מרפה.
- הכלל המלא, כפי שחודד: **כניסה למודול נוחתת על "ניהול כללי"; פתיחת לקוח מעבירה ללשונית שלו.**

## [22.1] - 2026-09-04 · **ניהול כללי — הספר של המאמן, ולקוח ריק**

### לשונית "ניהול כללי"
- **לשונית ראשונה, לא נמחקת, וכל כניסה למודול נוחתת עליה.** צבע אינדיגו שאי אפשר לתת לאף לקוח, ומשקולת מצוירת — היא מסך, לא לקוח.
- **לוח חודשי.** כל יום ריבוע לחיץ; ריבוע סגור מציג את **המקומות שאימנת בהם ("מקום - שירות")** בצבע של המקום, לא סכום. יותר משניים — `…`.
- **חלונית של יום** צפה מעל הריבוע שלו: מה נרשם בו, כמה נכנס, `+` להוספה, `×` לסגירה. מעבר ליום אחר או לחיצה בחוץ **שומרים** (כפתור השמירה נשאר).
- **"הוסף אימון"** — קופסה קבועה: מקום, שירות, מחיר **ותאריך**. נחת בחודש אחר? הלוח עובר לשם.
- **מחסן מקומות** — זיכרון בלבד שמזין השלמה אוטומטית בשדה השם; מקום מוכר ממלא שירות ומחיר מהפעם הקודמת. **מחיר חדש הוא לפעם הבאה** — עסקה שנעשתה שומרת את המחיר שנעשתה בו לנצח.
- **טבלת עסקאות** שנפתחת **מקובצת לפי מקום**: כמות בסוגריים, סכום, שירות (או "מגוון"), והטווח שנבחר. חץ לצד כל שם פותח את האימונים של אותו מקום מתחתיו. "פירוט מלא" מציג שורה לכל אימון.
- **סינון ומיון**: כל כותרת עמודה ממיינת (ולחיצה שנייה הופכת), רשימות של מקום ושירות שמסננות מיד, וטווחי השבוע/החודש/השנה/טווח תאריכים.
- **עמודת "חשבונית"** שאתה מסמן: סימון על מקום מסמן את כל האימונים בו, וביטול של אימון אחד מוריד את הסימון מהמקום.
- **הכנסות**: הכנסה מתוכניות + הכנסה אישית + סה״כ, באותם שלושה מספרים בלשונית ובחץ שבסרגל העליון.
- **תאריכים בפורמט ישראלי** — יום/חודש/שנה בשתי ספרות, בכל שדה ובכל טבלה.

### לקוח ריק
- **סוג לקוח שלישי.** ארבע שאלות — שם, מין, תשלום חודשי, אופן התשלום — ו**אורך לבנה** שהוא בוחר (4 כברירת מחדל).
- **חודש של ימים ריקים**: כל שבעת הימים פתוחים לעריכה, **אף אחד מהם אינו יום מנוחה**, ואין דילואד. אתה כותב, אתה מאשר, אתה שולח.
- **כל השאר זהה**: קוד, מכשיר, תקנון, שער אישור, מיילים (הצטרפות, שינוי של הלקוח, תזכורת סוף לבנה — לפי האורך האמיתי של הלבנה), הקפאה ומחיקה.
- **גדר סביב מוח המאמן:** אורך הלבנה חי במסלול של הסוג הזה בלבד. `lib/client-intake.js` לא מכיר את השדה, החוזה של המאמן עדיין 4 שבועות, ולבנת המשך קוראת את סוג הלקוח מהתוכנית לפני שהיא מכבדת אורך אחר. בדיקות נופלות אם זה משתנה.

### עוד
- **העתק/הדבק שבוע** בקליק ימני על קוביית השבוע — כל השבוע עובר בכתיבה אחת (הפאזה, שבה נרשם שבוע מנוחה, במכוון לא עוברת).
- שרת פיתוח מקומי מגיש עכשיו גם `/api/client-program` ו-`/api/admin-ledger`.

## [22.0] - 2026-09-03 · **תצוגת לקוחות — שלוש רגליים** (שוחרר כ-22.0; 21.7 מעולם לא עלה לאוויר)

### מה נסגר לקראת העלייה
- **מוח המאמן עונה לבעלים ולאיש מלבדו.** `/api/personal-coach` דורש אימות אדמין לפני שהבקשה נקראת בכלל — לפני כל קריאה לספק, כך שסירוב לא עולה כסף. האפליקציה לא מחזיקה יכולת AI אחת (POL-029).
- **מסלול AI מת נמחק:** `/api/security-coach` קרא לספק ולא היה לו קורא אחד בכל המוצר.
- **כלי שלב הבדיקות ירדו:** צ'קבוקס "Test program", תווית "בדיקה" על לשונית, וכפתור מילוי הלבנה באימוני דוגמה — עם הפעולה בשרת שמאחוריו. לא מוסתר, נמחק: כל מנגנון שמסוגל לכתוב תוכן ליום אימון הוא בדיוק מה ש-POL-029 אומר שלא יהיה.
- **לוח הזמנים של המתאמן הבודד** בתחקור בין הלבנים — אילו ימים, כמה דקות לכל יום, וכל כמה שבועות שבוע מנוחה — במקום הלשונית של סטודיו. מה שנענה שם מעצב את הלבנה שנבנית.
- **כותרות גרסה:** האפליקציה v22.0 וללא שורת גרסת המאמן (הציבור לא נחשף למאמן), מודול אדמין 4.0 עם שורת המאמן — שם היא רלוונטית.

### תצוגת לקוחות — שלוש רגליים

DUCK-WOD מפסיק להיות אפליקציית מאמן-AI בשירות עצמי והופך לשלוש רגליים: **האפליקציה** (אוספי אימונים), **מודול אדמין** (הפלטפורמה הניהולית — וכל יכולות ה-AI, שם ורק שם), ו**תצוגת לקוחות** (לינק לתוכנית שנכתבה ביד).

### POL-029 · תצוגת לקוחות
- **אדר כותב את התוכנית. המערכת נותנת את הכלי.** יצירת לקוח מפיקה שלד ריק — אין שורת אימון אחת שנוצרת אוטומטית.
- **אפס AI בצד הלקוח, כתכונה של הקוד ולא כהבטחה.** אין באף מודול של המוצר הזה מסלול לספק AI. הסתרת כפתור היא לא כיבוי — `/api/generate-workout` היה מוסתר בממשק שלוש גרסאות בזמן שהוא עוד ענה לאינטרנט.
- **ה-Blob הוא מקור האמת**, לא המכשיר. כל כתיבה נושאת מספר גרסה שנבדק בתוך מנעול. **ל-Blob אין כתיבה מותנית** — בלי זה מי ששומר אחרון מוחק את השני בשקט. שמירה מיושנת **מסורבת** ומקבלת את הגרסה החיה.
- **הלקוח אדון לעצמו** — עורך בחופשיות, והעריכה **נכנסת לתוקף מיד**. אין שער אישור: הוא נושא באחריות המקצועית, ושער היה מחזיר את אדר לשרשרת.
- **אדר מעודכן, לא נשאל** — דגל "לא נקרא" הוא **מצב, לא מונה**: חמש שמירות לאותו יום הן דגל אחד. מייל אחד לכל אימון שנגע, ואז שקט על אותו אימון.
- **גישה = זהות, לא לינק.** קוד חד-פעמי שאדר מנפיק ומוסר בעצמו. **אין מייל ואין טלפון של הלקוח במאגר.** קודים וטוקנים נשמרים כגיבוב מלוח בלבד. תקרת מכשירים. החתימה נרשמת על **החשבון** — מאמן עם פלאפון ומחשב חותם **פעם אחת**.
- **כל מה שעובר ללקוח בנוי כרשימת היתר.** נתוני התשלום ותור ה"לא נקרא" של אדר לא נוסעים לעולם.

### התנאים המשפטיים
- נוסח B2B חדש, `v3.4-legal` — אנגלית מחייבת + תרגום לעברית, מילה במילה כפי שנמסר. נפרד לגמרי מתנאי המתאמנים (`v2.0-legal`).
- הנוסח יושב בקובץ אחד שגם הדף וגם השרת קוראים — עותק שני של טקסט משפטי הוא עותק שיכול לסטות.
- בדיקות מצמידות כל סעיף נושא-אחריות, כך ששיפוץ ניסוח יפיל את החבילה.

### האפליקציה — חוזרת להיות אגרגטור
- **לשונית המאמן האישי הוסרה** יחד עם הפאנל ושני החלונות הצפים. נחיתה על "Workouts".
- לינקים ישנים (`?tab=pprog`) **לא נשברים** — מנותבים.
- **בדיקת גרסה מינימלית** חדשה: אפליקציה מותקנת ישנה תקבל "עדכן" במקום מסך שבור. הרצפה נקבעה מתחת לגרסה הנוכחית בכוונה — אף אחד לא מוטרד היום.

### הצד של אדר
- **דף ניהול אחד.** מסך הלקוחות הוא מסך בתוך `admin.html`, לא עמוד נפרד — `admin-clients.html` נשאר רק כהפניה שלא שוברת לינקים ישנים. בתוכו: יצירת לקוח, כתיבת האימונים, הנפקת קוד, רשימת מכשירים וביטול, דגל "לא נקרא", **סכום חודשי כמספר עם סה"כ**, שיטת תשלום כמלל, שינוי שם בעיפרון, ובניית אינדקס מחדש.
- **תיקון הסיסמה:** הכשלון השקט שגרם לך להקליד סיסמה בכל כניסה הופך לגלוי — המסך אומר אם "זכור אותי" פעיל, ומצביע על `ADMIN_SESSION_SECRET` החסר.
- **אינדקס** במקום סריקה שהורידה כל תוכנית במלואה — רשימת האדמין הייתה נשברת בסביבות 50 לקוחות.
- **הרענון האוטומטי הפסיק להוריד את כל המתאמנים (2 בספטמבר).** הדף רענן את עצמו כל 20 שניות, ורשימת מתאמנים נבנית מקריאה מלאה של כל מתאמן — כולל הלבנה שלו. זה השעה את מאגר ה-Blob באמצע יום בדיקות. עכשיו הרענון שואל שאלה אחת קטנה — "השתנה משהו?" — ורק תשובה חיובית מושכת את הרשימה: כ-80 קריאות קטנות בשעה במקום כ-2,300 מלאות, והמחיר לא גדל ככל שיהיו יותר לקוחות.
- **תחקור מתאמן קצה נפתח.** שתי קריסות באותו מקום — שדה שנמחק, ואחר כך שם מקוצר שלא הוגדר — השאירו את התחקור פתוח על המסך ומת. עכשיו יש בדיקה שמפעילה אותו באמת ועוברת את שמונת השלבים.

### בדיקות
43 קבצי מבחן, 1961 בדיקות. חדשים: מחסן התוכניות (כולל **מרוץ אמיתי** של עשר שמירות בו-זמנית), גבול האבטחה, שכבת הגישה, הנוסח המשפטי, ה-API מקצה לקצה, המסכים החדשים, הסרת המאמן מהאפליקציה, **מדידת מחיר הרענון מול מאגר אמיתי** (נכשלת אם רענון חוזר לגדול עם מספר המתאמנים), ו**הרצה אמיתית של התחקור** על דפדפן מדומה — שתופסת שם חסר לפני שהוא מגיע למסך.

## [21.6] - 2026-08-30 · **the athlete adjusts today; the coach owns the brick**

### מוטת השליטה של המתאמן (POL-028)
- **המאמן הצף הוסר.** אחרי התחקור, המתאמן שולט באימון של היום בלבד — חלופות, חזרות, סטים, משקלים, תרגיל מוחלף — דרך התיבה שמתחת לאימון. **הלבנה שייכת למאמן האנושי** במודול האדמין.
- החסימה **בשרת, לפני כל קריאה ל-AI** (`lib/coach-athlete-scope.js`) — בקשה חסומה עולה אפס. הסתרת כפתור היא לא כיבוי.
- **התחקור לא נגע.** הוא הדרך היחידה שמתאמן מקבל תוכנית, וכך גם מילוי התוכנית האוטומטי. למאמן האנושי — גישה מלאה ללבנה.
- **הרווח בעלות:** POL-COST-004/005/006 קיימים כדי לרסן בדיוק את הלחץ הזה. הוא יצא מהישג ידו של המתאמן. *לא* סוגר את חור התקציב — המונים עדיין מגיעים מהמכשיר.

### המנוע הישן (Generate Workout) — נמחק לגמרי
- היה כבוי בממשק מאז 21.2.3, אבל `/api/generate-workout` עדיין ענה לכל מי שידע את הכתובת: בלי סיסמה, בלי תנאים, בלי תקרת עלות. מתג כבוי, חוט חי.
- נמחקו: הקובץ, הרישום ב-Vercel, מסלולי שרת הפיתוח, הכפתור, הפאנל וכל החיווט. הקוד נשמר בהיסטוריית גיט. בדיקות מוודאות שהוא **יישאר** מחוק.

### חוסן — שלוש תקלות אמיתיות
- **תיקון תקיעה מוחלטת בבניית שבוע.** תשובה חתוכה מ-Gemini הכניסה את קוד ההצלה ללולאה אינסופית: הגלגל הסתובב עד שהשרת נכנע, והקריאה כבר חויבה. אומת מול הקוד הישן — נתקע ונהרג אחרי 8 שניות.
- **תקלת רשת כבר לא מפילה את השרת.** קריאות Gemini עטופות, ולנקודת הקצה יש רשת ביטחון שמחזירה הודעה ידידותית במקום קריסה גולמית.
- **"בוצע" בלי שינוי — תוקן.** קורא הנתונים של שינוי יום בודד קיבל את אותה סובלנות שכבר הייתה לשבוע ולבלוק (עטיפת קוד, תשובה חתוכה, אותיות גדולות/קטנות). שינוי שהמתאמן אישר כבר לא נופל בשקט.

### סנכרון אדמין ↔ אפליקציה
- אחרי POL-028, משיכת העריכות של המאמן האנושי היא **הדרך היחידה שתוכנית משתנה**. עד כה היא קרתה בשתי נקודות בלבד — ואחת מהן הייתה פתיחת המאמן הצף שהוסר. נוספה משיכה בכל חזרה של האפליקציה לחזית (מווסתת ל-20 שניות).

### ניקיון מוח המאמן
- הוסרו 4 קבצים בני ~150 בתים שכל תוכנם היה הפניה — הם היו נכנסים למאגר הידע כזבל.

### בדיקות
- 24 קבצי מבחן, 559+ בדיקות. שני קבצים חדשים: `coach-json-extract.test.js` (מנעול הלולאה) ו-`coach-athlete-scope.test.js` (POL-028 — כולל מה שחייב **להמשיך** לעבוד).
...
- **Coach subtitle:** `COACH · v2.3` (app display stays **v21.5.1**).
- **Calendar:** after Confirm on an unplanned/extra session, Israel-today shows as **LOGGED** (not REST DAY); tomorrow Rest. `loggedExtra*` survives normalize / week_detail.
- **Chat:** when the last coach bubble is **Confirm?** / אישור, show a one-tap **כן** button (decline stays free-text).
...
- Shipped into **Coach 2.3** (see above). Preserve `loggedExtra*` through normalize / week_detail; LOGGED UI.
