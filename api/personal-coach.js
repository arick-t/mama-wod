 ...
 *   2.3.5 — POL-027 clarify: thrusters valid CF; anti-spam ≠ ban (balance, don’t exclude)
...
const PROGRAMMING_SYSTEM_CORE =
  ...
  "Each training week: ≥1 lunge-family + (indoors) ≥1 wall pattern; thrusters are valid CF — anti-spam ≠ ban (don’t thruster-spam). " +
...
module.exports = async function handler(req, res) {
  ...
  if (body.brickChat === true || body.wholeProgramChat === true) {
    ...
    try {
      if (body.intakeBaseline) {
        intakeBaseSnap = JSON.stringify(body.intakeBaseline).slice(0, 5500);
      }
    } catch (eBase) {
      intakeBaseSnap = "";
    }
    systemText +=
      ...
      todayIso +
      ".\n" +
      "POL-024 PROCESS (HARD) on every WHOLE-BRICK adaptation (not single-day pre-talk):\n" +
      "A) Silently re-read FULL INTAKE BASELINE — every section (profile, equipment/setup, weekly schedule, " +
    ...
    messages = [
      {
        ...
        text:
          ...
          "Each training week: include a lunge-family pattern and (if indoors) a wall pattern; thrusters are valid CF — avoid thruster-spam (not a ban). " +
  ...
  if (action === "revise_week") {
    ...
    try {
      if (body.intakeBaseline) {
        intakeBaseRw = JSON.stringify(body.intakeBaseline).slice(0, 5500);
      }
    } catch (eIb) {
      intakeBaseRw = "";
    }
