...
- **Required behavior:** Max **2** programmed edits per Israel `sessionDate` (calendar day). After the cap: notes/preferences only — **no** new programming JSON. Soft lock reply (English, short): today’s session is locked after 2 edits; can save a preference for tomorrow/next week. Safety/technique questions may continue without JSON.
- **Added:** 2026-08-03 — cost guardrails
...
- **Required behavior:** Soft Upgrade only: scan remaining days (active week ± next) → propose **≤3 patches** + Confirm? → `revise_day` / part surgical only. Max **one Soft Upgrade per brick**. Does **not** count as Large Rebuild unless the athlete explicitly chooses B / scope becomes week-wide.
...
- **Type:** HARD
- **Scope:** when Daily / Large / Soft caps are hit
- **Trigger:** further rewrite demand while capped
- **Required behavior:** Acknowledge briefly, save preference if useful, **refuse programming JSON**, suggest the next-window surgical edit. English, short, no long apology.
- **Added:** 2026-08-03 — cost guardrails
