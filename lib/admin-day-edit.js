...
(function (root, factory) {
  ...
  function applyPendingToDay(dayData, pending, ctx) {
    ...
    next.modifiedPartKinds = mergeModifiedKinds(
      dayData && dayData.modifiedPartKinds,
      pending.modifiedPartKinds
    );
  ...
  function protectPendingDayParts(existingBlock, incomingBlock, pending) {
    ...
    week.days[dayKey] = Object.assign({}, clientDay, {
      parts: exDay.parts,
      modifiedPartKinds: exDay.modifiedPartKinds || clientDay.modifiedPartKinds || {},
    });
    next.weeks[wi] = week;
    return next;
  }
  ...
  function statusMessage(pending) {
    ...
    if (pending.status === "pending") return "ממתין לסנכרון למכשיר";
    if (pending.status === "applied") return "הוחל במכשיר";
