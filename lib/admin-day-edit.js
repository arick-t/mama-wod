...
(function (root, factory) {
  if (typeof module === "object" && module.exports) {
    module.exports = factory(
      ...
      require("./day-rest-toggle.js")
    );
  } else {
    root.AdminDayEdit = factory(root.NormalizePprogBlock, root.PprogDisplay, root.DayRestToggle);
  }
})(typeof self !== "undefined" ? self : this, function (NormalizePprogBlock, PprogDisplay, RestToggle) {
  "use strict";
  ...
  function lockReason(dayKey, dayData, week, dayIso, todayIso, phase) {
    ...
    /* A rest day used to be locked here (21.7 opened it). A rest day is a PLAN, not
     * a fact: replacing it with a session is one of the most ordinary edits there is,
     * and blocking it meant the pencil either never appeared or appeared and then
     * refused to save.
     *
     * The remaining locks are a different kind of thing — they all describe a day
     * that has already HAPPENED (an extra session logged, a finish report filed, a
     * debrief written, a date in the past). Those stay locked, because editing them
  ...
  function applyPendingToDay(dayData, pending, ctx) {
    ...
    /* Writing a session onto a day whose week overview still says "Rest" produces a
     * day that holds real programming and still renders as a rest day — the edit
     * looks applied and looks like it did nothing. isRestDay() reads the overview
     * FIRST, so the focus has to move with the parts.
     *
     * Reported rather than silently mutated, because the caller owns the week object
     * and is the one that persists it. */
    var focusHint = null;
    if (RestToggle && ctx.week && !RestToggle.partsAreRest(next.parts)) {
      if (RestToggle.isRestFocusText(RestToggle.overviewFocus(ctx.week, dayKey))) {
        var firstTitle = String(((next.parts || [])[0] || {}).title || "").trim();
        focusHint = firstTitle || RestToggle.DEFAULT_TRAINING_FOCUS;
        /* ctx.week is the live week the caller is about to save — move it here so a
           caller that forgets focusHint still gets a correct result. */
        RestToggle.setOverviewFocus(ctx.week, dayKey, focusHint);
      }
    }
    return { ok: true, day: next, focusHint: focusHint };
  }
