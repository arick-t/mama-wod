...
function buildIntakeCompleteMail(snapshot) {
  ...
  const blockSummary = String(block.summaryLine || "4-week training block").slice(0, 240);
...
function snapshotReadyForJoinMail(snapshot) {
  ...
  if (snapshot.intakeNotifySent || snapshot.joinMailSent) return false;
  if (snapshot.deleted || snapshot.revoked) return false;
  const block = snapshot.currentBlock;
  if (!block || !Array.isArray(block.weeks) || !block.weeks.length) return false;
  const blockStart = String(block.blockStart || "").trim();
  if (!/^\d{4}-\d{2}-\d{2}/.test(blockStart)) return false;
...
async function sendAdminIntakeCompleteMail(snapshot) {
  if (snapshot && (snapshot.intakeNotifySent || snapshot.joinMailSent)) {
