...
(function (root, factory) {
   ...
   */
  /**
   * "סכום לחשבונית קרובה" — what is on screen and has no invoice tick yet.
   *
   * It sits beside the table's own heading because that is the list it is counting, and
   * it says which range it counted so the number can never be read as meaning something
   * wider than it does (owner, 2026-09-08).
   *
   * @param {{amount:number, rangeLabel?:string}} opts
   */
  function dueBoxHtml(opts) {
    ...
    return (
      ...
      "</div>" +
      (o.rangeLabel ? '<div class="led-due-range">' + esc(o.rangeLabel) + "</div>" : "") +
      "</div>"
    );
  }
