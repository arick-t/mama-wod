...
(function (root, factory) {
  ...
  const COLUMNS = [
    ...
    /* WHAT this line is: something he did once, or a client who pays every month.
       It is the difference between a gig and an income, and it was the one thing the
       table could not tell him (owner, 2026-09-22). */
   ...
   */
  function headerHtml(monthKey, total, thisMonth) {
    /* "היום" — the way back after browsing months (owner, 2026-09-09). It is always
       there, so the header does not change shape as he moves around; on the month he
       is already looking at it is quiet and flashes today's square instead of
       reloading anything. */
    const onThisMonth = !!thisMonth && String(monthKey) === String(thisMonth);
    return (
      '<div class="led-head">' +
      ...
      '<button type="button" class="led-today' + (onThisMonth ? " is-here" : "") +
      '" data-led-today="1" title="חזרה לחודש הנוכחי">היום</button>' +
  ...
  function calendarHtml(opts) {
    ...
    days.forEach(function (iso) {
      ...
      const lines = rows.slice(0, LINES).map(function (d) {
        ...
        /* A pale yellow frame around a client who pays every month, so a standing
           income is never mistaken for a session he happened to give somewhere
           (owner, 2026-09-22). */
        const kind =
          (d.nature === "recurring" ? " is-recurring" : "") +
          (d.projected === true ? " is-promised" : "");
        return '<span class="led-line' + kind + '"' + style + ">" + esc(label) + "</span>";
      });
  ...
  function tableHtml(rows, total, colours, sort, opts) {
    ...
    html +=
      ...
      list
        .map(function (d) {
          if (!grouped) return dealRowHtml(d, colours, o);
          ...
          return (
            ...
            kids
              .map(function (x) {
                return dealRowHtml(x, colours, o);
              })
              .join("") +
            "</div>"
          );
  ...
  /** One session. */
  function dealRowHtml(d, colours, opts) {
    const o = opts && typeof opts === "object" ? opts : {};
    const recurring = d.nature === "recurring";
    /* A bill that has not reached its day yet. It is on the calendar so he can see
       what is coming, but it is not a record: there is nothing to invoice and nothing
       to edit until the day arrives (owner, 2026-09-22). */
    const promised = d.projected === true;
    return (
      '<div class="led-row' + (recurring ? " is-recurring" : "") +
      (promised ? " is-promised" : "") + '" data-led-deal="' + esc(d.id) + '" data-led-month="' +
      esc(String(d.day).slice(0, 7)) + '"' + colourStyle(colours, d.name) + ">" +
      '<span class="led-cell led-name">' + esc(d.name) + "</span>" +
      /* On a monthly client the service is the only thing about the arrangement he can
         type, and this row is where he sees it — so this is where he changes it
         (owner, 2026-09-22). Everything else on the line is a record. */
      '<span class="led-cell led-service" style="color:' + serviceColour(d.service) + '">' +
      (recurring && d.clientId
        ? '<button type="button" class="led-service-edit" data-led-sub-service="' + esc(d.clientId) +
          '" title="על מה החיוב">' + esc(d.service || "—") + "</button>"
        : esc(d.service || "—")) + "</span>" +
      '<span class="led-cell led-nature">' + esc(natureLabel(d.nature)) + "</span>" +
      '<span class="led-cell led-price">' + esc(shekel(d.price)) +
      /* A price already agreed for the next bill, in brackets beside the one being
         charged now — so he can see the change coming without opening anything. */
      pendingHtml(d, o) + "</span>" +
      '<span class="led-cell led-date">' + esc(hebDate(d.day)) + "</span>" +
      '<span class="led-cell led-inv">' +
      (promised
        ? '<span class="led-promised" title="החיוב עוד לא הגיע">צפוי</span>'
        : '<input type="checkbox" data-led-invoiced="' + esc(d.id) + '" data-led-month="' +
          esc(String(d.day).slice(0, 7)) + '"' + (d.invoiced ? " checked" : "") +
          ' aria-label="הופקה חשבונית">') +
      "</span>" +
      "</div>"
    );
  }

  /** "חד פעמי" / "חזרתי" — his own two words for it. */
  function natureLabel(nature) {
    return nature === "recurring" ? "חזרתי" : "חד פעמי";
  }

  /**
   * The coming price, beside the one on the row.
   *
   * Only where there IS one: a price he changed from the next bill onwards leaves the
   * current bill alone, and this is the only place that says so before the date comes.
   */
  function pendingHtml(d, o) {
    const map = o && o.pending && typeof o.pending === "object" ? o.pending : {};
    const hit = d && d.clientId ? map[d.clientId] : null;
    if (!hit || !hit.price) return "";
    if (d.day >= String(hit.from || "")) return "";
    return ' <span class="led-next-price" title="מהחיוב ב-' + esc(hebDate(hit.from)) +
      '">(' + esc(shekel(hit.price)) + ")</span>";
  }
  ...
  function groupRowHtml(g, colours, o) {
    return (
      ...
      (g.mixed
        ? esc("מגוון")
        : g.nature === "recurring" && g.clientId
          ? '<button type="button" class="led-service-edit" data-led-sub-service="' + esc(g.clientId) +
            '" title="על מה החיוב">' + esc(g.service || "—") + "</button>"
          : esc(g.service || "—")) + "</span>" +
      /* A place he goes to and a client who pays monthly can both sit under one name,
         so a grouped line says which it is only when the whole group agrees. */
      '<span class="led-cell led-nature">' + esc(g.nature ? natureLabel(g.nature) : "—") + "</span>" +
      '<span class="led-cell led-price">' + esc(shekel(g.total)) +
      /* The table OPENS grouped, so the coming price has to be readable here too. Only
         where the line is ONE bill: on a sum of several months a second number beside
         the total would say nothing true (owner, 2026-09-22). */
      (g.count === 1 && g.clientId
        ? pendingHtml({ clientId: g.clientId, day: g.lastDay }, o)
        : "") + "</span>" +
  ...
  function filtersHtml(opts) {
    ...
    return (
      ...
      services
        ...
        .join("") +
      "</select>" +
      /* "חד פעמי" / "חזרתי" — the two kinds of line, so he can look at his standing
         income without the gigs in the way (owner, 2026-09-22). */
      '<select class="led-field" id="ledFNature" data-led-filter="nature">' +
      '<option value="">כל הסוגים</option>' +
      '<option value="once"' + (o.nature === "once" ? " selected" : "") + ">חד פעמי</option>" +
      '<option value="recurring"' + (o.nature === "recurring" ? " selected" : "") + ">חזרתי</option>" +
      "</select>" +
   ...
   */
  /**
   * "סכום לחשבונית קרובה" — everything he has not invoiced, since ever.
   *
   * NOT the range on screen and NOT what any filter is showing: the number's whole job
   * is that nothing is ever missed, so a date range beside it would only invite the
   * reading that it counts less than it does (owner, 2026-09-08).
   *
   * @param {{amount:number}} opts
   */
  function dueBoxHtml(opts) {
    const o = opts || {};
    const amount = Number(o.amount) || 0;
    const clear = amount <= 0;
    return (
      '<div class="led-due' + (clear ? " is-clear" : "") + '">' +
      '<div class="led-due-title">סכום לחשבונית קרובה</div>' +
      '<div class="led-due-sum">' +
      (clear ? "הכל חויב" : "₪" + Math.round(amount).toLocaleString()) +
      "</div>" +
      "</div>"
    );
  }
  ...
  function blocksBoxHtml(opts) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      html +=
        ...
        /* The pencil belongs beside the NAME it changes, not at the far end of the row
           (owner, 2026-09-08). */
        "<td><strong>" + esc(r.name) + "</strong>" +
        '<button type="button" class="led-pen" data-block-name="' + esc(r.id) + '" title="שנה שם" aria-label="שנה שם">✎</button>' +
        ...
        /* Its name is what he finds it by, so it can be changed here (owner, 2026-09-08). */
  ...
  function placeKeyOf(name) {
    return String(name || "").trim().toLowerCase();
  }

  /**
   * One monthly client in the favourites box.
   *
   * Four things, in the order he reads them: who, what he pays, what for, and WHEN.
   * The billing date is the only editable field on the row, because it is the only one
   * whose home is the book — the name, the colour and the price are decided on his tab
   * and arrive here already settled (owner, 2026-09-22).
   */
  function subRowHtml(sub, o, swatches) {
    const frozen = sub.active === false;
    const editing = !frozen && o.editing && placeKeyOf(o.editing) === placeKeyOf(sub.name);
    if (editing) {
      return (
        '<div class="led-fav-row is-recurring is-editing" data-led-fav="' + esc(sub.name) + '">' +
        '<input class="led-field" id="ledFavName" maxlength="80" value="' + esc(sub.name) + '" readonly ' +
        'title="השם נקבע בלשונית של הלקוח">' +
        '<div class="led-swatches">' +
        swatches
          .map(function (c) {
            return (
              '<button type="button" class="led-swatch' + (sub.colour === c ? " on" : "") +
              '" data-led-sub-colour="' + esc(c) + '" data-led-sub="' + esc(sub.clientId) +
              '" style="background:' + esc(c) + '" aria-label="' + esc(c) + '"></button>'
            );
          })
          .join("") +
        "</div>" +
        '<button type="button" class="led-cancel" data-led-fav-cancel="1">סגור</button>' +
        "</div>"
      );
    }
    return (
      '<div class="led-fav-row is-recurring' + (frozen ? " is-frozen" : "") +
      '" data-led-fav="' + esc(sub.name) + '" data-led-sub="' + esc(sub.clientId) + '"' +
      colourStyle({ x: sub.colour }, "x") + ">" +
      '<span class="led-fav-name">' + esc(sub.name) +
      (frozen ? ' <span class="led-fav-frozen">מוקפא</span>' : "") + "</span>" +
      '<span class="led-fav-uses">כל חודש</span>' +
      '<span class="led-fav-meta">' + esc(sub.service || "—") + " · " + esc(shekel(sub.price)) +
      (sub.pending && sub.pending.price
        ? ' <span class="led-next-price" title="מהחיוב ב-' + esc(hebDate(sub.pending.from)) + '">(' +
          esc(shekel(sub.pending.price)) + ")</span>"
        : "") +
      "</span>" +
      /* The day of the month the money falls on. Typed the way he writes a date
         everywhere else in this book — day first. */
      '<span class="led-fav-billing">' +
      (frozen
        ? '<span class="led-fav-billing-day">' + esc(billingLabel(sub)) + "</span>"
        : '<button type="button" class="led-fav-billing-btn" data-led-billing="' + esc(sub.clientId) +
          '" title="תאריך חיוב חודשי">' + esc(billingLabel(sub)) + "</button>") +
      "</span>" +
      '<span class="led-rowacts">' +
      (frozen
        ? ""
        : '<button type="button" class="led-mini" data-led-fav-edit="' + esc(sub.name) + '" title="צבע">✎</button>') +
      "</span>" +
      "</div>"
    );
  }

  /** "ה-5 בחודש" — how he says a billing day out loud. */
  function billingLabel(sub) {
    const n = Number(sub && sub.billingDay);
    if (!Number.isFinite(n) || n < 1 || n > 31) return "—";
    return "ה-" + n + " בחודש";
  }

  /**
   * Everyone he works for — and now, at the top, everyone who pays him every month.
   *
   * A place is somewhere he went; a subscription is an arrangement that stands. They
   * share this box because the question he brings to it is the same one ("who are my
   * people and what are they worth"), but they are not the same thing, so a monthly
   * client carries the one field a place has no answer to: the day of the month the
   * money falls on. A place shows a dash there (owner, 2026-09-22).
   *
   * A frozen client stays in the list, grey and untouchable — he asked for that in the
   * same breath as the freeze itself: freezing is not deleting.
   */
  function favouritesBoxHtml(opts) {
    ...
    /* A monthly client whose name is also a place is ONE line, not two: the
       arrangement is the fuller answer, so it wins. */
    const claimed = {};
    subs.forEach(function (sub) {
      claimed[placeKeyOf(sub.name)] = true;
    });
    const rest = places.filter(function (p) {
      return !claimed[placeKeyOf(p.name)];
    });
    let html =
      '<button type="button" class="led-fold" data-led-fav-toggle="1" aria-expanded="' +
      (o.open ? "true" : "false") +
      '"><span class="led-fold-caret">' + (o.open ? "▾" : "▸") + "</span>" +
      '<span>מועדפים</span><span class="led-fold-count">' + esc(subs.length + rest.length) + "</span></button>";
    if (!o.open) return html;
    if (!subs.length && !rest.length) {
      html += '<p class="led-empty">עוד לא נתת שירות לאף אחד.</p>';
      return html;
    }
    html +=
      '<div class="led-fav-rows">' +
      subs.map(function (sub) { return subRowHtml(sub, o, swatches); }).join("") +
      rest
        .map(function (p) {
          ...
          return (
            ...
            /* A place is not billed on a date — it is somewhere he went. */
            '<span class="led-fav-billing is-none">—</span>' +
            '<span class="led-rowacts">' +
