...
(function (root, factory) {
  ...
  /**
   * "Add a session" — the same three fields, plus the day they belong to.
   *
   ...
   *
   * @param {{name?: string, service?: string, price?: number, date?: string}} opts
   */
  function manualFormHtml(opts) {
    ...
    return (
      ...
      '<button type="button" class="led-field led-pick" id="ledMName" data-led-pick="manual">' +
      (o.name ? esc(o.name) : '<span class="led-hint">שם המקום…</span>') +
      "</button>" +
      '<input class="led-field" id="ledMService" maxlength="80" placeholder="שירות" value="' + esc(o.service || "") + '">' +
      '<input class="led-field led-num-field" id="ledMPrice" type="number" min="1" max="100000" step="1" placeholder="מחיר" value="' + esc(priceStr) + '">' +
  ...
  function tableHtml(rows, total, colours) {
    ...
    html +=
      ...
      list
        .map(function (d) {
          return (
            '<div class="led-row" data-led-deal="' + esc(d.id) + '" data-led-month="' + esc(String(d.day).slice(0, 7)) + '"' + colourStyle(colours, d.name) + ">" +
            '<span class="led-cell led-name">' + esc(d.name) + "</span>" +
            '<span class="led-cell led-service">' + esc(d.service || "—") + "</span>" +
            '<span class="led-cell led-price">' + esc(shekel(d.price)) + "</span>" +
