...
/**
 * How stale a device's "last seen" may get before the stamp is worth a write.
 * A convenience for the owner's device list is not worth a write per visit.
 */
const LAST_SEEN_REFRESH_MS = 6 * 60 * 60 * 1000;

/**
 * Stamp one device's "last seen" onto a FRESHLY READ access row.
 *
 * Why this exists rather than writing back the row the request already held:
 * Blob has no conditional write, so putJson is a blind overwrite of the whole row.
 * The read path held a copy taken before a slow programme read and wrote it back
 * afterwards, which silently undid anything written in that window — a code the
 * owner had just issued, or a device that had just redeemed one. The client was
 * then told their device was not authorised, and only a new code got them back in
 * (owner, 2026-09-15).
 *
 * So: change one field on a row read moments ago, and skip the write entirely when
 * the stamp is still fresh — which is most of the time.
 *
 * @returns {{ changed: boolean, access: object, missing?: true }}
 */
function touchDevice(freshAccess, deviceId, opts) {
  const o = isPlainObject(opts) ? opts : {};
  const next = normalizeAccess(freshAccess, o.programId, o.now);
  const id = String(deviceId || "");
  const at = nowMs(o.now);
  const every = Number.isFinite(Number(o.everyMs)) ? Number(o.everyMs) : LAST_SEEN_REFRESH_MS;
  for (const d of next.devices) {
    if (!d || d.id !== id) continue;
    const last = Date.parse(String(d.lastSeenAt || ""));
    if (Number.isFinite(last) && at - last < every) return { changed: false, access: next };
    d.lastSeenAt = new Date(at).toISOString();
    next.updatedAt = d.lastSeenAt;
    return { changed: true, access: next };
  }
  /* The device is not on the fresh row — revoked, or pushed out by the cap. Nothing
     to stamp, and above all nothing to write: putting this row back would be the
     very overwrite this function exists to avoid. */
  return { changed: false, access: next, missing: true };
}
