...
(function (root, factory) {
  ...
  var MAX_AGE = 90;
  /**
   * ABILITY — ticks, but one room has one ability, so they exclude each other.
   *
   * "Mixed ability AND beginners" reaches the coach as two contradictory facts about one
   * room, which is the failure this whole round has been closing. Mixed is what most
   * rooms are, so it leads in a box of its own and is ticked before anything is touched
   * (owner, 2026-09-15).
   */
  var LEVEL_DEFS = [
    { id: "mixed", label: "Mixed ability", lead: true },
  ...
  /**
   * KIND OF GROUP — ticks that combine, because they genuinely do: a women-only
   * pre-army group is one room, not two.
   *
   * "General fitness" leads in a box of its own and is ticked to begin with, but it does
   * NOT clear the others: most rooms are general fitness AND something (owner,
   * 2026-09-15).
   *
   * "Youth / academy" is not here: the age range above already says it, and a room that
   * answers the same thing twice is a room whose two answers can disagree.
   */
  var GROUP_TYPE_DEFS = [
    { id: "general_fitness", label: "General fitness", lead: true },
  ...
  function emptyIntake() {
    return {
      ...
      ageFrom: 0,
      ageTo: 0,
      /* The answers most rooms give, so they are the answers the tab opens on. */
      levels: { mixed: true },
      groupTypes: { general_fitness: true },
  ...
  function normalizeIntake(raw) {
    ...
    /* A map now. An intake answered while this was a single string still means what it
       said: the string becomes the one tick it always was. */
    out.levels = {};
    if (isPlainObject(r.levels)) {
      LEVEL_DEFS.forEach(function (d) {
        if (r.levels[d.id] === true) out.levels[d.id] = true;
      });
    } else if (typeof r.level === "string" && r.level) {
      LEVEL_DEFS.forEach(function (d) {
        if (d.id === r.level) out.levels[d.id] = true;
      });
    }
    out.groupTypes = {};
  ...
  function validateIntake(raw) {
    ...
    if (!Object.keys(v.levels).length) {
  ...
  function buildStudioIntakePrompt(raw) {
    ...
    var lines = [
      ...
      /* A room is never asked whether it has tested its members' maxima: it does not
         happen, and a field nobody will ever tick is a field that can only be ticked by
         mistake (owner, 2026-09-15). LOAD BASIS states the rule in the prompt itself, and
         it applies to every room. */
  ...
  function populationFacts(v) {
    ...
    var lvls = LEVEL_DEFS.filter(function (d) {
      return v.levels && v.levels[d.id] === true;
    }).map(function (d) {
      return d.label.toLowerCase();
    });
    if (lvls.length) out.push(lvls.join(", "));
