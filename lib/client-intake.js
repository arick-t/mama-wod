...
(function (root, factory) {
  ...
  function emptyIntake() {
    return {
      ...
      equipmentOther: "",
      /* How many people can work at once, and on what. The binding constraint in a
         room of 12-20 is the number of STATIONS, not the brand of equipment: 18 people
         on 6 barbells is a different session from 18 on 18, and it decides whether a
         squat can be written at all (coach agent, 2026-09-02). Free text on the
         owner's instruction - he will not count in boxes. */
      stations: "",
  ...
  function normalizeIntake(raw) {
    ...
    out.stations = clampText(r.stations, 600);
  ...
  function buildStudioIntakePrompt(raw) {
    ...
    var lines = [
      ...
      String(v.stations || "").trim()
        ? "STATIONS: " + String(v.stations).trim()
        : "STATIONS: not stated - do NOT assume one station per person. Prefer designs that work with sharing and rotation.",
      "",
