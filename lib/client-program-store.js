...
function createProgramStore(io) {
   ...
   */
  function copiedPart(p, id) {
    const src = isPlainObject(p) ? p : {};
    const out = {
      id: id,
      title: String(src.title || ""),
      lines: Array.isArray(src.lines) ? src.lines.slice() : [],
    };
    const noteLines = parseInt(src.noteLines, 10);
    if (Number.isFinite(noteLines) && noteLines > 0) out.noteLines = noteLines;
    /* The shape travels with the copy, or the copy is read differently from the day it
       was copied from (owner, 2026-09-05). */
  ...
  /** A whole day, copied whole — its parts and the name he gave it. */
  function copiedDay(srcDay, weekNumber, dayKey) {
    ...
    const out = {
      parts: parts.map(function (p, i) {
        return copiedPart(p, "w" + weekNumber + dayKey + i);
      }),
    };
    const title = srcDay && typeof srcDay.title === "string" ? srcDay.title.trim() : "";
