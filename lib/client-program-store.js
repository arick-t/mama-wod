...
function emptyWeek(absoluteWeekIndex, intake, legacyLastWeek, deloadSince) {
  ...
  /* The deload is a cadence read off the absolute week index, and nothing else makes a
     week a deload. The old path used to call the LAST week of a block a deload whenever
     there was no intake to ask — which is the rule the owner retired: a deload exists
     only because someone asked for one (owner, 2026-09-22, POL-032). */
...
function newBlock(opts) {
  ...
  return {
    ...
    /* A name he gave the month — "כוח בסיסי" — shown after its dates. It travels
       through every rearrangement, because newBlock is what rebuilds a block record
       (owner, 2026-09-08). */
...
function appendEmptyBlock(draft, opts) {
  ...
  /* A new month continues the shape he is working in NOW: if he cut the client to three
     sessions a week, the next block is three too, not whatever was sold a season ago
     (owner, 2026-09-08). Only where a week actually says so — nothing is invented. */
  let carrySessions = 0;
  for (let i = weeks.length - 1; i >= 0; i--) {
    const n = parseInt(weeks[i] && weeks[i].sessions, 10);
    if (n >= 1 && n <= 7) { carrySessions = n; break; }
  }
  for (let i = 1; i <= added; i++) {
    const fresh = emptyWeek(last + i, intake, false, deloadSince);
    if (carrySessions) fresh.sessions = carrySessions;
    weeks.push(fresh);
  }
  draft.weeks = weeks;
...
function emptyProgram(opts) {
  const o = isPlainObject(opts) ? opts : {};
  const weeks = [];
  /* Four, like every other month. It said five, from the time a brick ran to a fifth
     deload week (owner, 2026-09-22, POL-032). */
  const weekCount = Math.max(1, Math.min(12, parseInt(o.weekCount, 10) || 4));
...
function createProgramStore(io) {
  ...
  /**
   * Rename a block on the shelf.
   *
   * The name is what he finds it by — "כוח למתאמן מתחיל" rather than the week it came
   * from (owner, 2026-09-08). Both the row in the index and the block itself, so the
   * table and what gets planted never disagree.
   */
  async function renameWarehouseBlock(id, name) {
    if (!safeId(id)) return { ok: false, code: "BAD_ID", error: "unusable id" };
    const clean = String(name == null ? "" : name).trim().slice(0, 80);
    if (!clean) return { ok: false, code: "NO_NAME", error: "לבנה במחסן חייבת שם" };
    const got = await readWarehouseBlock(id);
    if (!got.ok) return got;
    const entry = Object.assign({}, got.block, { name: clean });
    try {
      await store.putJson(warehouseKey(id), entry);
    } catch (e) {
      return { ok: false, code: "WRITE_FAILED", error: String((e && e.message) || e) };
    }
    const idx = await readWarehouseIndex();
    const rows = (idx.ok ? idx.rows : []).map(function (r) {
      return r && r.id === id ? Object.assign({}, r, { name: clean }) : r;
    });
    try {
      await store.putJson(WAREHOUSE_INDEX_KEY, {
        version: 1,
        updatedAt: new Date(nowMs(clock)).toISOString(),
        rows: Warehouse.sortRows(rows),
      });
    } catch (e) {
      return { ok: false, code: "INDEX_FAILED", error: String((e && e.message) || e) };
    }
    return { ok: true, id: id, name: clean };
  }
  ...
  async function pasteBlock(programId, expectedVersion, payload, opts) {
    ...
    const r = await updateProgram(
      ...
      expectedVersion,
      function (draft) {
        const made = appendEmptyBlock(draft, {
          weekCount: srcWeeks.length,
          notes: o.notes,
          name: isPlainObject(payload) ? payload.name : "",
        });
        added = made.added;
    ...
    const r = await updateProgram(
      ...
      function (draft) {
        ...
        const copyBlock = newBlock({
          ...
          /* A copy of a month he named is that month again — he can rename it. */
          name: src.block.name,
          approvedAt: null,
        });
    ...
    const r = await updateProgram(
      ...
      function (draft) {
        ...
        const block = newBlock({
          ...
          createdAt: new Date(nowMs(clock)).toISOString(),
          intake: intake,
          /* A block off the shelf arrives under the name it was saved as. */
          name: payload.name,
          approvedAt: null,
        });
  ...
  /**
   * How many sessions a week the client trains, FROM a given week onwards.
   *
   * He sold four a week, they now train five: the weeks already written keep the four
   * they were written with, and the change starts where he made it (owner, 2026-09-05).
   * Nothing is deleted either way — a session that stops being shown is still on file,
   * so putting the number back brings it back.
   *
   * @param {number} fromWeek1 the first week the new number applies to, 1-based
   * @param {number} sessions 1..7
   */
  async function setWeekSessions(programId, expectedVersion, fromWeek1, sessions) {
    const from = parseInt(fromWeek1, 10);
    const want = parseInt(sessions, 10);
    if (!(want >= 1 && want <= 7)) {
      return { ok: false, code: "BAD_SESSIONS", error: "אימונים בשבוע הוא מספר בין 1 ל-7" };
    }
    let changed = 0;
    const r = await updateProgram(
      programId,
      ...
      function (draft) {
        const weeks = Array.isArray(draft.weeks) ? draft.weeks : [];
        if (!(from >= 1) || from > weeks.length) throw new Error("no such week");
        for (let i = from - 1; i < weeks.length; i++) {
          if (!isPlainObject(weeks[i])) continue;
          weeks[i].sessions = want;
          changed += 1;
        }
        return draft;
      },
    ...
    if (!r.ok) return r;
    return { ok: true, program: r.program, version: r.version, changed: changed, sessions: want };
  }

  /** Name a block, or take its name off with an empty string. */
  async function renameBlock(programId, expectedVersion, blockIndex1, name) {
    const want = parseInt(blockIndex1, 10);
    ...
    const r = await updateProgram(
      ...
      function (draft) {
        ...
        let hit = null;
        for (const b of blocks) {
          if ((parseInt(b && b.blockIndex, 10) || 0) === want) hit = b;
        }
        if (!hit) throw new Error("no such block");
        hit.name = clean;
        return draft;
      },
      { actor: "owner" }
    ...
    if (!r.ok) return r;
    return { ok: true, program: r.program, version: r.version, name: clean };
  }
