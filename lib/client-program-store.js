...
/* The one session seedTestBlock writes. Deliberately identical on every day and
   deliberately labelled: nobody should ever mistake it for programming. */
const SAMPLE_SESSION = [
  {
    title: "Part A — SAMPLE (test program)",
    lines: [
      "12 min duration / strength priority",
      "Every 2:00 x 6 sets:",
      "3 Back squats @ 70%",
      "Cue: brace before you break at the knee.",
    ],
  },
  {
    title: "Part B — SAMPLE (test program)",
    lines: ["10 min duration / engine priority", "AMRAP 10:", "200m run", "10 KB swings", "10 push-ups"],
  },
];
...
function createProgramStore(io) {
   ...
   */
  /**
   * Fill a block with one sample session, on every day that is not a rest day.
   *
   * This exists so the owner can look at a whole month of the calendar at once and
   * judge the DISPLAY, which he cannot do against four empty weeks. It is not a
   * generator and it is not a shortcut around POL-029: it writes the same fixed lines
   * everywhere, it says so on every part, and it REFUSES on any program that is not
   * marked as a test. A paying client's program can never be filled by a machine.
   */
  async function seedTestBlock(programId, expectedVersion, blockIndex) {
    const current = await readProgram(programId, { allowCache: false });
    if (!current.ok) return current;
    if (current.program.isTest !== true) {
      return {
        ok: false,
        code: "NOT_A_TEST_PROGRAM",
        error: "only a program marked as a test can be filled with a sample session",
      };
    }
    let filled = 0;
    const r = await updateProgram(
        ...
        for (const b of draft.blocks) {
          ...
          for (let w = start; w <= end; w++) {
            ...
            for (const d of DAY_KEYS) {
              ...
              day.parts = SAMPLE_SESSION.map(function (p, i) {
                return { id: "w" + w + d + i, title: p.title, lines: p.lines.slice() };
              });
              filled++;
            }
          }
    ...
    if (!r.ok) return r;
    return Object.assign({ filled: filled }, r);
  }
