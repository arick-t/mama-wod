...
const DAY_FIELDS = ["parts", "modified", "coachModified"];
...
function parseClientEdit(body) {
  ...
  for (const e of rawEdits) {
    ...
    if (wantsRest) { edits.push({ weekIndex: weekIndex, dayKey: dayKey, rest: true, parts: [] }); continue; }
...
function applyClientEdit(draft, parsed) {
  ...
  for (const edit of parsed.edits) {
    ...
    if (isPlainObject(day)) day.modified = true;
