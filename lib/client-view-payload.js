...
/* "noteLines" says how many of the leading lines were written as notes — without it
   ...
   them back. */
/* "formatLine" completes the SHAPE of a part beside "noteLines": how many leading
   lines are notes, and whether the one after them is the format line. Without it the
   card guesses by English words, and a Hebrew session is guessed wrong
   (owner, 2026-09-05). */
const PART_FIELDS = ["id", "title", "lines", "modified", "noteLines", "formatLine", "lineColors", "numbered", "lineNums"];
...
/* "sessions" is how many sessions THIS week holds — a client who moved from four a
   week to five keeps the weeks he already trained as they were (owner, 2026-09-05). */
const WEEK_OUT = ["weekIndex", "phase", "theme", "summaryLine", "overview", "days", "sessions"];
...
function cleanPart(part) {
  ...
  const formatLine = parseInt(out.formatLine, 10);
  if (formatLine === 1 || formatLine === 0) out.formatLine = formatLine;
  else delete out.formatLine;
  /* A number he wrote over the count. Anything that is not a small whole number is
     dropped — the line then simply counts itself. */
  const nums = {};
  let anyNum = false;
  if (isPlainObject(out.lineNums)) {
    for (const key of Object.keys(out.lineNums).slice(0, 60)) {
      const idx = parseInt(key, 10);
      const val = parseInt(out.lineNums[key], 10);
      if (!Number.isFinite(idx) || idx < 0 || idx >= out.lines.length) continue;
      if (!Number.isFinite(val) || val < 0 || val > 999) continue;
      nums[String(idx)] = val;
      anyNum = true;
    }
  }
  if (anyNum) out.lineNums = nums;
  else delete out.lineNums;
...
/** Hidden by the owner: written, kept, and not for this client's eyes. */
function isHiddenDay(day) {
  return !!(isPlainObject(day) && day.hidden === true);
}

/**
 * How many sessions a week this client can actually see.
 *
 * The questionnaire's number is what the coach was told to WRITE; hiding a session
 * changes what is SENT. Taken as the widest week of the ones they can see, so the grid
 * keeps one shape across the block and a week with one fewer shows a gap rather than
 * re-flowing under them (owner, 2026-09-16).
 */
function visibleSessionColumns(weeks, sold, anyHidden) {
  /* Nothing hidden, nothing to narrow: the number stays exactly what it has always
     been, so a programme nobody has hidden anything in cannot change shape. An empty
     month still says "three a week" — it is the shape they bought. */
  if (!anyHidden) return sold;
  let widest = 0;
  for (const w of weeks) {
    let n = 0;
    for (const k of DAY_KEYS) {
      const day = w && w.days && w.days[k];
      if (day && Array.isArray(day.parts) && day.parts.length) n++;
    }
    if (n > widest) widest = n;
  }
  if (!widest) return 0;
  return Math.min(widest, sold);
}
...
function programForClient(program) {
  ...
  /* Which language their programme is written in, ALWAYS stated — a page that has to
     guess is a page that draws the month one way on a first load and another on a
     refresh. English unless the owner said otherwise (owner, 2026-09-16). */
  out.outputLanguage = program.outputLanguage === "he" ? "he" : "en";
  out.weeks = [];
  const visible = Store.approvedWeekCount(program);
  const weeks = (Array.isArray(program.weeks) ? program.weeks : []).slice(0, visible);
  /* Whether the owner has hidden anything at all in what they can see. Nothing hidden
     means nothing about their calendar changes — see visibleSessionColumns. */
  let anyHidden = false;
  for (const w of weeks) {
    ...
    week.days = {};
    for (const k of DAY_KEYS) {
      /* A HIDDEN day does not reach the client at all — not as an empty card, not as a
         gap with a note on it. The owner writes more sessions than he sells and hides
         the ones a client has stopped taking, because an extra session costs a fraction
         and a missing one costs a month (owner, 2026-09-16). Same cut as an unapproved
         block, one size smaller, and made HERE rather than in the page so that calling
         the endpoint directly gives the same answer. */
      if (isHiddenDay(w.days && w.days[k])) {
        anyHidden = true;
        week.days[k] = cleanDay(null);
        continue;
      }
    ...
    if (Array.isArray(week.overview)) {
      week.overview = week.overview
        /* A hidden day's FOCUS is the day, one line shorter. Left in, the client's page
           builds a placeholder card out of it — "Planned focus · Full session details
           still loading…" — with the session's own name on it, which is the hidden day
           announcing itself (owner, 2026-09-20). Cut here with the parts, at the same
           boundary, for the same reason. */
        .filter(function (o) {
          return !isHiddenDay(w.days && w.days[String((o && o.day) || "")]);
        })
        .slice(0, 7)
        .map(function (o) {
          return {
  ...
  if (isPlainObject(program.intake) && program.intake.scheduleMode === "session_count") {
    ...
    const sold = Number.isFinite(n) && n >= 1 && n <= 7 ? n : 0;
    /* What they were sold, narrowed to what is left after hiding. A room that bought
       three and has one hidden sees two columns, not three with a hole. */
    out.sessionColumns = sold ? visibleSessionColumns(out.weeks, sold, anyHidden) : 0;
  }
  out.blockGroups = [];
  ...
  for (const b of blocks) {
    ...
    out.blockGroups.push({
      ...
      /* The name of the month, so the client reads the plan by the same name the coach
         calls it (owner, 2026-09-08). */
 ...
 */
/**
 * THE HIDDEN ENGLISH SURVIVES A CLIENT'S OWN EDIT.
 *
 * A Hebrew programme keeps its English out of sight in `linesEn`, because English is
 * the only language the mechanical check can read. That field is NOT in PART_FIELDS
 * and so never crosses to the client — which means what the client sends back has no
 * English in it, and writing it straight over the stored day erased it. The client
 * changing one line left the whole day unchecked (owner, 2026-09-20).
 *
 * So the English is taken from the day AS STORED and matched to the returning lines
 * by their text. A line the client left alone keeps its English; a line the client
 * CHANGED keeps none, because English describing a movement that is no longer there
 * is worse than no English at all.
 */
function carryEnglishBack(storedDay, parts) {
  const stored = (isPlainObject(storedDay) && Array.isArray(storedDay.parts) ? storedDay.parts : []);
  const byText = {};
  const titles = {};
  let any = false;
  for (const p of stored) {
    ...
    const en = Array.isArray(p.linesEn) ? p.linesEn : [];
    for (let i = 0; i < lines.length; i++) {
      const he = String(lines[i] == null ? "" : lines[i]).trim();
      const eng = String(en[i] == null ? "" : en[i]).trim();
      if (he && eng) {
        byText[he] = eng;
        any = true;
      }
    }
    const t = String(p.title == null ? "" : p.title).trim();
    if (t && p.titleEn) {
      titles[t] = String(p.titleEn);
      any = true;
    }
  }
  if (!any) return parts;
  return parts.map(function (p) {
    ...
    out.linesEn = lines.map(function (l) {
      const he = String(l == null ? "" : l).trim();
      return byText[he] || he;
    });
    const t = String(p.title == null ? "" : p.title).trim();
    if (titles[t]) out.titleEn = titles[t];
    return out;
  });
}
  ...
  for (const edit of parsed.edits) {
    ...
    if (edit.rest === true) {
      ...
      RestToggle.makeSession(
        ...
        edit.dayKey,
        carryEnglishBack(
          isPlainObject(week.days) ? week.days[edit.dayKey] : null,
