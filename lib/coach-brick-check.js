...
(function (root, factory) {
  ...
  /**
   * Everything a part says, in the language a machine can read.
   *
   * `linesEn` when it is there — the English kept out of sight beside a Hebrew screen —
   * and the shown lines otherwise. Every check in this file reads a part through here,
   * so none of them can go blind when the product speaks Hebrew (owner, 2026-09-16).
   */
  function partText(part) {
    var en = part && part.linesEn;
    var lines = (Array.isArray(en) && en.length ? en : (part && part.lines)) || [];
    return String((part && (part.titleEn || part.title)) || "") + " " + lines.join(" ");
  }
    ...
    weeks.forEach(function (w, wi) {
      ...
      DAY_KEYS.forEach(function (d) {
        ...
        parts.forEach(function (p) {
          ...
          /* THE ENGLISH, WHEN THERE IS A SEPARATE COPY OF IT.
             A programme written for a Hebrew-speaking room shows Hebrew and nothing else
             — "10 מתח" reads better than "10 מתח (Pullup)" and the owner asked for the
             brackets gone. The English then lives in `linesEn`, out of sight, and THIS is
             what reads it: a check that can only see English would otherwise go blind the
             moment the product speaks Hebrew, which is how a whole week of sit-ups
             written "22 כפיפות בטן" was reported as having no bodyweight work in it
             (owner, 2026-09-16). */
          var shown = (p && p.lines) || [];
          var lines = Array.isArray(p && p.linesEn) && p.linesEn.length ? p.linesEn : shown;
          ...
          if (!lines.length && p && p.title) {
            out.push({ week: idx, day: d, text: String(p.titleEn || p.title), part: p });
          }
        });
      });
    });
   ...
   *
   * THE one thing the owner rewrote by hand, line by line, in the first real studio
   * brick: "75% 1RM", "78%", "82-85%" written for seventeen-year-olds who had never
   * tested a lift. LOAD BASIS says it in the prompt and the prompt was obeyed in every
   * other respect and not in this one — so it is checked here too, where it is certain
   * (owner, 2026-09-15).
   *
   * Certain because the caller only passes false when there is genuinely no number: a
   * room never has one, and a person who reported lifts is allowed percentages and is
   * never checked here at all.
   */
  function percentViolations(lines, allowed) {
    if (allowed !== false) return [];
    var out = [];
    var seen = null;
    lines.forEach(function (l) {
      /* A load percentage, not a stray number: 30-100 and written as a percentage. */
      var m = /(?:^|[^0-9])([3-9][0-9]|100)%/.exec(String(l.text || ""));
      if (!m) return;
      if (seen) {
        seen.count++;
        return;
  ...
  function sentence(v) {
    ...
    if (v.kind === "percent") {
      return (
        "A percentage of a 1RM was prescribed (" + v.wrote + ")" +
        (v.count > 1 ? " " + v.count + " times" : "") +
        " (" + v.where + ": \"" + v.example + "\") and NOBODY HERE HAS A TESTED MAXIMUM. " +
        "A percentage has nothing to be a percentage of. Prescribe effort instead: an effort " +
        "out of ten for strength and the olympic lifts, reps in reserve for gymnastics."
      );
    }
   ...
   *                                 percentagesAllowed: false when no maximum was ever tested }
  ...
  function checkBrick(block, ctx) {
    ...
    violations = violations.concat(percentViolations(lines, c.percentagesAllowed));
