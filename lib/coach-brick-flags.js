 ...
 * @param {object} [opts]   { agent } — kept for future per-agent checks. The studio exemption on
 *                          percentages was REMOVED on 2026-09-14: it was the second half of the
 *                          same bug, and it is why nobody was warned about עודד's brick
 * @returns {string[]} at most MAX_FLAGS short English lines
 */
function brickFlags(block, profile, opts) {
  const flags = [];
  ...
  /* A scale written as the prescription — the owner's under-scaling point, 2026-09-08.
     Only movements whose standard is unambiguous, and only when the standard is NOT also
     named in the same part: 'Pull-ups (or ring rows)' is correct and must not flag. */
  const scalesAsRx = [];
  function noteScale(name) {
    if (scalesAsRx.indexOf(name) < 0) scalesAsRx.push(name);
  }
  ...
  block.weeks.forEach(function (w, wi) {
    ...
    DAY_KEYS.forEach(function (d) {
      ...
      parts.forEach(function (p) {
        const t = partsText(p);
        /* No agent is exempt. A room whose members have never tested a lift is exactly the case
           the flag is for — and the exemption is what kept it silent for עודד (owner, 2026-09-14). */
        if (reported === 0 && /%\s*1RM|@\s*\d+\s*%/i.test(t)) sawPercent = true;
        if (/\b\d+\s*(inch|inches|ft|foot|feet|lb|lbs)\b/i.test(t)) sawImperial = true;
        /* Collect every distinct one, so a second scale is not masked by the first — they still
           ride on ONE flag line. */
        if (/\bring rows?\b/i.test(t) && !/\bpull.?ups?\b/i.test(t)) noteScale("ring row");
        if (/\b(banded|band-assisted|jumping) pull.?ups?\b/i.test(t)) noteScale("assisted pull-up");
        if (/\bsingle.?unders?\b/i.test(t) && !/\bdouble.?unders?\b/i.test(t)) {
          noteScale("single-under");
        }
      });
    });
  });
  ...
  if (scalesAsRx.length) {
    flags.push(
      "Scaled movement(s) written as the prescription: " +
        scalesAsRx.join(", ") +
        ". The standard belongs on the line."
    );
  }
  if (sawImperial) {
