 ...
 * @param {object} [opts]   { agent } — a studio is exempt from the percentage check, because a
 *                          room has no shared 1RM and %1RM is how you write load for one
...
function brickFlags(block, profile, opts) {
  ...
  const isStudio = !!(opts && opts.agent === "studio");
  ...
  block.weeks.forEach(function (w, wi) {
    ...
    DAY_KEYS.forEach(function (d) {
      ...
      parts.forEach(function (p) {
        const t = partsText(p);
        if (!isStudio && reported === 0 && /%\s*1RM|@\s*\d+\s*%/i.test(t)) sawPercent = true;
