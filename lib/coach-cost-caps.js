 ...
 */
function evaluateCostCapGate(action, body, profile) {
  ...
  if (a === "generate_block" && body && (body.largeRebuild === true || body.midBrickRebuild === true)) {
    if (isLargeRebuildLocked(state, today)) {
      return {
        code: "COST_CAP_LARGE",
  ...
  if (a === "revise_week") {
    if (body && body.softUpgrade === true && state.softUpgradeUsedForBrick === true) {
      return {
        code: "COST_CAP_SOFT",
    ...
    if (body && body.largeRebuild === true && isLargeRebuildLocked(state, today)) {
      return {
        code: "COST_CAP_LARGE",
        error: "Large rebuild cap reached",
        hint:
