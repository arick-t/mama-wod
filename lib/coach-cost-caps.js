...
function monthlyUsedFromState(state) {
  const st = state && typeof state === "object" ? state : {};
  if (typeof st.monthlyUnitsUsed === "number") return st.monthlyUnitsUsed | 0;
  if (st.monthlyLocked === true) return MONTHLY_CAP;
  const mk = israelMonthKey(st.israelToday);
  const map = st.monthlyUnits && typeof st.monthlyUnits === "object" ? st.monthlyUnits : null;
  if (map && map[mk] != null) return map[mk] | 0;
  return 0;
}

function recordMonthlyUnits(caps, units, todayIso) {
  const c =
    caps && typeof caps === "object" ? Object.assign(emptyCostCaps(), caps) : emptyCostCaps();
  if (!c.monthlyUnits || typeof c.monthlyUnits !== "object") c.monthlyUnits = {};
  if (!c.dailyEdits || typeof c.dailyEdits !== "object") c.dailyEdits = {};
  const mk = israelMonthKey(todayIso);
  c.monthlyUnits[mk] = (c.monthlyUnits[mk] | 0) + (units | 0);
  return c;
}

function recordBrickFill(caps, todayIso) {
  return recordMonthlyUnits(caps, BRICK_FILL_UNITS, todayIso);
}
...
function evaluateCostCapGate(action, body, profile, opts) {
  ...
  if (today && !state.israelToday) state.israelToday = today;

  const monthlyUsed = monthlyUsedFromState(state);
