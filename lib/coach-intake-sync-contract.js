...
(function (root, factory) {
  ...
  function normalizeIntakeProfile(raw) {
    ...
    return {
      ...
      activeRecoveryDay:
        s.activeRecoveryPref === "yes" ? String(s.activeRecoveryDay || "thu").slice(0, 8) : "",
      skills: skills,
      lifts: lifts,
  ...
  function applyIntakeProfileToPhoneStore(pkg, intakeProfile) {
    ...
    pkg.legalAcceptedVersion = Math.max(pkg.legalAcceptedVersion | 0, 3);
    if (!pkg.legalAcceptedAt) pkg.legalAcceptedAt = new Date().toISOString();
    return pkg;
  }
