...
(function (root, factory) {
  ...
  function buildFixedIntakePrompt(s) {
    ...
    function inventoryFor(profile) {
      ...
      var list = asObj(profile && profile.equipmentList);
      ...
      return Catalog.inventoryText(list, {
        room: false,
        answered: Object.keys(list).length > 0,
        extraText: String((profile && profile.trainingLocationOther) || "").trim(),
      });
    }
    ...
    return [
      ...
      multiPlace
        ? (secondNamed
            ? "Primary (" + (primaryDayLabels || "the rest of the week") + "): " +
              (s.trainingSetup || "unknown") + "\n" +
              "Also trains (" + secondDayLabels + "): " + (secondKit || "not described.")
            : "Primary: " + (s.trainingSetup || "unknown") + "\n" +
              "Also trains: days not stated - treat the reported setup as the primary one." +
              (secondKit ? "\n  What is there: " + secondKit : "")) +
          "\nLOAD: TWO SETTINGS. Prescribe by %1RM from the reported lifts on the primary days." +
          (secondNamed
            ? " On " + secondDayLabels + " prescribe from the implements listed above."
            : " Prescribe from implements only where the second setting applies.") +
          " NAME THE SETTING in the session itself."
        : s.trainingSetup || "unknown",
      "",
      "WEEKLY SCHEDULE:",
