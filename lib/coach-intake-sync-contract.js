...
(function (root, factory) {
  if (typeof module === "object" && module.exports) {
    ...
    /* admin.html and claim.html must load lib/equipment-catalog.js BEFORE this file. */
    root.CoachIntakeSync = factory(root.EquipmentCatalog);
  }
   ...
   *
   * Goals were free text until 2026-09-15, and the reason to mark them is not enforcement
   * — no code can check whether a month made someone stronger. It is ROUTING: "I want to
   * get stronger" in prose reached the coach as nothing at all, because no word in it was
   * one the router recognised (coach agent, 2026-09-02). The free line stays beside these,
   * unlike the equipment step, because it reaches the owner rather than widening a
   * permission nothing can check.
   *
   * TWO AT MOST. Not for cost — ten words against 103,574 characters of instruction is
   * nothing — but because three pull volume, intensity and recovery in three directions
   * inside one month, and what comes back is a compromise serving none of them. A brick is
   * a month, not a decision for life (owner, 2026-09-15).
   *
   * "Maintaining a healthy lifestyle" leads and clears the rest: it is the answer most
   * people give, and it is an answer, not the absence of one.
   */
  var MAX_GOALS = 2;
  var GOAL_DEFS = [
    { id: "healthy_lifestyle", label: "Maintaining a healthy lifestyle", exclusive: true },
    { id: "build_muscle", label: "Build muscle" },
    { id: "lose_fat", label: "Lose fat and lean out" },
    { id: "max_strength_1rm", label: "Max strength (1RM)" },
    { id: "explosive_power", label: "Explosive power and speed" },
    { id: "muscular_endurance", label: "Muscular endurance - long sets, unbroken" },
    { id: "engine", label: "Engine and cardio endurance" },
    { id: "run_faster", label: "Run faster - 5K / 10K" },
    { id: "specific_skill", label: "A specific skill", needsDetail: true },
  ];
  /* Injury-proofing, mobility and coming back after a break were on this list for an hour.
     The owner took them off (2026-09-15): this product does not speak about health,
     rehabilitation or injury in that register at all. What an athlete must not do is asked
     where it belongs and can be acted on — the movement families on step 7. */

  /** Which skill, when "a specific skill" is the goal. The eight people actually ask for. */
  var GOAL_SKILL_DEFS = [
    "First pull-up",
    "Muscle-up",
    "Handstand",
  ...
  var PROFILE_DEFS = [
    ...
    {
      id: "experience",
      /* "10" alone reached the coach as ten of something - days, months, years. The unit
         belongs in the question (coach agent, 2026-09-03). */
      label: "Training experience (years)",
      kind: "text",
      placeholder: "e.g. 2, or: 2 years functional fitness",
    },
  ];
  ...
  var INTAKE_PROFILE_KEYS = [
    ...
    /* More than one place to train. A box on weekdays and a garage on Saturday is one
       athlete in TWO settings, and describing it as "limited equipment" threw away the
       four maxima he had just reported (coach agent, 2026-09-03). */
    "trainsMultipleLocations",
    "secondaryLocationDays",
    /* The second place is a second INVENTORY as of 2026-09-15, ticked from the same
       catalogue with ceilings of its own. The sentence and the single "heaviest there"
       number it replaces stay in the shape for intakes answered before that day. */
    "secondaryEquipmentList",
    "secondaryLocationEquipment",
    "secondaryHeaviestImplementKg",
    /* One number per training day, only when he says the days differ. */
    "sessionMinutesByDay",
    /* The individual chooses their own deload cadence; a studio manages its own. */
    "deloadEveryWeeks",
    "improveFocus",
    "improveFocusOther",
    "avoidMovements",
    "avoidMovementsOther",
    /* Replaced by the per-implement checklist on 2026-09-14 — see equipmentList. */
    "equipmentList",
    "avoidInProgram",
    "injuries",
    "goals",
  ...
  /**
   * Which marked skills a movement limit cancels.
   *
   * The owner's ruling, 2026-09-03: "if a user said he has a movement limitation, that is
   * the clause that wins - not the fact that he has all the skills or the right kit."
   * A pistol IS a deep squat; a muscle-up IS hanging and kipping. Marked Rx-capable and
   * forbidden at the same time is a contradiction the packet must settle, not hand over
   * (coach agent, 2026-09-03).
   */
  var AVOID_CANCELS_SKILLS = {
    deep_squat: ["pistol"],
    hinge_deadlift: [],
    overhead_press: ["handstand_pushup", "handstand_walk"],
    hanging_bar: ["muscle_up", "ring_muscle_up", "toes_to_bar", "pullups", "chest_to_bar"],
    kipping: ["muscle_up", "ring_muscle_up", "toes_to_bar", "pullups", "chest_to_bar"],
    jumping: ["double_unders"],
    running: [],
  };

  /** The skills a set of avoided movements cancels, as labels, without repeats. */
  function skillsCancelledBy(avoidMap, skillsMap) {
    var out = [];
    var seen = {};
    var marked = isObj(skillsMap) ? skillsMap : {};
    var avoided = isObj(avoidMap) ? avoidMap : {};
    var all = marked.all_skills === true;
    for (var i = 0; i < AVOID_MOVEMENT_DEFS.length; i++) {
      var id = AVOID_MOVEMENT_DEFS[i].id;
      if (avoided[id] !== true) continue;
      var cancels = AVOID_CANCELS_SKILLS[id] || [];
      for (var j = 0; j < cancels.length; j++) {
        var sid = cancels[j];
        if (!all && marked[sid] !== true) continue;
        if (seen[sid]) continue;
        seen[sid] = 1;
        for (var k = 0; k < SKILL_DEFS.length; k++) {
          if (SKILL_DEFS[k].id === sid) out.push(SKILL_DEFS[k].label);
        }
      }
    }
    return out;
  ...
  function buildFixedIntakePrompt(s) {
    ...
    /* The individual chooses their own cadence; a studio manages its own. Until this
       field existed every individual packet said "no deload week falls inside this block"
       - months of building with no down week (coach agent, 2026-09-03). */
    var deloadIdx = deloadWeekIndexForBlock(s.blockStartWeek, s.deloadEveryWeeks, BLOCK_WEEKS);
    /* Both lists, one map. The marks are the goals as of 2026-09-15; an intake answered
       before that carries the old improve-focus ids, and must keep meaning what it meant. */
    var improveLabels = markedLabels(
      GOAL_DEFS.concat(
        /* Two ids live on both lists. Without this the same goal is printed twice, once
           in each list's wording. */
        IMPROVE_FOCUS_DEFS.filter(function (d) {
          return !GOAL_DEFS.some(function (g) {
            return g.id === d.id;
          });
        })
      ),
    ...
    /* One athlete, so no station counts: what they own is a yes/no, and what matters beyond
       that is the CEILING per implement. Same renderer as the studio's, so both products speak
       one dialect of the same fact. */
    function inventoryOf(list, extraText) {
      if (!Catalog || typeof Catalog.inventoryText !== "function") return "";
      var l = asObj(list);
      /* The form writes a row per item, yes or no, so rows at all means the checklist was
         answered — and an athlete who ticked nothing gets an inventory of NOT AVAILABLE
         rather than the old "full gym" sentence below (owner, 2026-09-15). */
      return Catalog.inventoryText(l, {
        room: false,
        answered: Object.keys(l).length > 0,
        extraText: String(extraText || "").trim(),
      });
    }
    /** Was a checklist filled in for this place at all? Rows at all is the answer. */
    function answeredList(list) {
      return Object.keys(asObj(list)).length > 0;
    }
    function inventoryFor(profile) {
      return inventoryOf(
        profile && profile.equipmentList,
        profile && profile.trainingLocationOther
      );
    }
    /**
     * Two places, two inventories, each headed by the days it owns.
     *
     * Only when the second place was really answered — days named AND a list ticked.
     * Anything less and the single inventory stands, because a second place the coach
     * cannot locate in the week is worse than no second place at all.
     */
    function twoInventories(profile) {
      if (!multiPlace || !secondNamed) return "";
      var first = inventoryFor(profile);
      var second = inventoryOf(profile && profile.secondaryEquipmentList, "");
      if (!first || !second) return "";
      return [
        "FIRST PLACE - " + (primaryDayLabels || "every other training day") + ":",
        first,
        "",
        "SECOND PLACE - " + secondDayLabels + ":",
        second,
        "",
        "THE TWO LISTS ARE NOT INTERCHANGEABLE. A movement may be written into a day only if " +
          "the place that day belongs to lists its implement, and a ceiling belongs to the " +
          "place it was stated in.",
      ].join("\n");
    }
    /* TWO SETTINGS. A box on weekdays and a garage on Saturday is one athlete training in
       two rooms, and calling that "limited equipment" printed four maxima in kilograms and
       then forbade kilograms underneath them (coach agent, 2026-09-03). */
    var multiPlace = s.trainsMultipleLocations === true;
    var secondDays = Array.isArray(s.secondaryLocationDays) ? s.secondaryLocationDays : [];
    var secondDayLabels = secondDays
      .map(function (d) {
        return DAY_LABELS[d] || d;
      })
      .join(", ");
    var primaryDayLabels = days
      .filter(function (d) {
        return secondDays.indexOf(d) < 0;
      })
      .map(function (d) {
        return DAY_LABELS[d] || d;
      })
      .join(", ");
    var secondKg = parseInt(s.secondaryHeaviestImplementKg, 10) || 0;
    var secondKit = String(s.secondaryLocationEquipment || "").trim();
    /* Days named, or not named at all - and a guess is not an answer. */
    var secondNamed = multiPlace && secondDays.length > 0;
    var cancelled = skillsCancelledBy(s.avoidMovements, s.skills);
    var minutesByDay = isObj(s.sessionMinutesByDay) ? s.sessionMinutesByDay : {};
    var perDay = days
      .filter(function (d) {
        return parseInt(minutesByDay[d], 10) > 0;
      })
      .map(function (d) {
        return (DAY_LABELS[d] || d) + " " + parseInt(minutesByDay[d], 10);
      });
    /* Which room this is, because the answer to "how heavy does it get" means
       different things in each. In a proper box the question is not even asked, so a
       zero there is "no ceiling", not "unknown" - and the coach agent caught that the
       old wording told it "never by a kg figure" for an athlete who had just reported
       fourteen lifts in kg (coach agent, 2026-09-03). */
    var locsForLoad = isObj(s.trainingLocations) ? s.trainingLocations : {};
    var limitedRoom =
      locsForLoad.other_home === true ||
      (!locsForLoad.functional_gym && /^other|home or limited/i.test(String(s.trainingSetup || "")));
    return [
      ...
      "Experience: " +
        (String(s.experience || "").trim()
          ? /^\d+(\.\d+)?$/.test(String(s.experience).trim())
            ? String(s.experience).trim() + " years"
            : String(s.experience).trim()
          : "unknown"),
      "",
      "TRAINING SETUP:",
      multiPlace
        ? (secondNamed
            ? "FIRST PLACE (" + (primaryDayLabels || "every other training day") + ")\n" +
              "SECOND PLACE (" + secondDayLabels + ")\n" +
              "A training day belongs to ONE of them, and the split above is the whole of it: " +
              "any training day not named as the second place happens in the first."
            : "FIRST PLACE: every training day.\n" +
              "SECOND PLACE: reported, but no days were named - treat the first place as the " +
              "whole week rather than guessing which days moved.") +
          /* An intake answered before the second list existed described that place in a
             sentence. It is the only thing the coach has for those clients, so it is still
             carried — additive, in the owner's words, and never in place of a ticked list. */
          (secondKit && !answeredList(s.secondaryEquipmentList)
            ? "\nSECOND PLACE, as it was described before there was a list for it: " + secondKit
            : "") +
          "\nLOAD: TWO SETTINGS. NAME THE SETTING in the session itself, and write each day " +
          "only from the inventory of the place that day belongs to."
      ...
      s.activeRecoveryPref === "yes" && s.activeRecoveryDay
        ? "YES — one lighter ACTIVE RECOVERY DAY on " +
          (DAY_LABELS[s.activeRecoveryDay] || s.activeRecoveryDay) +
          ", the day they named. Other training days stay full sessions. This is NOT a deload: a deload is a whole week."
        /* Asked for, but no weekday named. Thursday used to be filled in here, which is
           a day the athlete never chose (owner, 2026-09-22, POL-032). */
        : s.activeRecoveryPref === "yes"
          ? "ASKED FOR, BUT NO WEEKDAY WAS NAMED — do NOT pick one yourself. Keep every training day a full session until they say which day."
          : "NO — do NOT program an active recovery day, and do not make any weekday lighter of your own accord.",
      ...
      "",
      "SESSION LENGTH:",
      /* A note in a free box ("weekends can reach 60") survived as prose and the coach
         flattened the week to 45, losing the only long session there was. One number per
         day when he says they differ (coach agent, 2026-09-03). */
      perDay.length
        ? perDay.join(" \u00b7 ") + " minutes (each a ceiling, not a target)"
        : s.sessionMinutes
      ...
      "",
      "GOALS:",
      /* The free line under the goal marks is gone (owner, 2026-09-15); for an individual
         this section is empty on purpose and the marks below carry the intent. A studio,
         and any intake answered before that day, still fills it. */
      s.goals || "stated as marks below, not in prose.",
      ...
      improveLabels.length
        ? "BLOCK GOALS: " + improveLabels.join(" · ") +
          " — at most two are ever chosen, and they LEAD the month: spend the dedicated " +
          "strength and skill time here."
      ...
      avoidLabels.length
        ...
        : "AVOID: none marked.",
      /* The box under the movement families held things like "no burpees after the knee"
         and reached the coach nowhere at all. Its own line, and present in both
         directions like every other - the invariant it asked for in the first place
         (coach agent, 2026-09-03). */
      /* A stated movement limit wins over a marked skill - the owner's ruling, and the
         packet settles it rather than presenting two lines and hoping (2026-09-03). */
      cancelled.length
        ? "AVOID OVERRIDES THESE MARKED SKILLS: " + cancelled.join(", ") +
          ". The athlete marked them Rx-capable; the avoid list wins. Do not program them in the form that needs the avoided pattern."
        : "AVOID OVERRIDES THESE MARKED SKILLS: none - no marked skill needs an avoided pattern.",
      String(s.avoidMovementsOther || "").trim()
        ? "AVOID (also): " + String(s.avoidMovementsOther).trim()
        : "AVOID (also): nothing else stated.",
      /* The ticked inventory when the athlete filled one, and the old sentence when they did
         not — an intake answered before 2026-09-14 must keep meaning exactly what it meant.
         The single "heaviest implement" number it replaces could not say "dumbbells to 15 but
         a 40 kg sandbag", and its absence used to tell a fully equipped athlete to write no
         kilograms at all, cancelling the %1RM table for the four maxima just reported. */
      twoInventories(s) ||
        inventoryFor(s) ||
        (multiPlace
          ? "HEAVIEST IMPLEMENT: primary setting is a full gym - prescribe by %1RM from the reported lifts." +
            (secondKg > 0
              ? "\nHEAVIEST IMPLEMENT" + (secondNamed ? " (" + secondDayLabels + ")" : " (second setting)") +
                ": " + secondKg + " kg."
              : "")
          : limitedRoom
    ...
    return [
      ...
      "Active recovery: " +
        (s.activeRecoveryPref === "yes"
          ...
          : "no — no active recovery day"),
      /* The brick is FOUR weeks, and the deload is whatever the intake chose - including
         none at all, which is a complete and legitimate answer (owner, 2026-09-20). The old
         line here promised a fifth week that is always a deload; it survived from a time when
         a brick was five weeks long, and it contradicted the questionnaire that replaced it. */
      "Deload: " +
        (parseInt(s.deloadEveryWeeks, 10) > 0
          ? "every " + parseInt(s.deloadEveryWeeks, 10) + " weeks, counted across the plan"
          : "none requested - do not place a deload week unless asked"),
      "Age: " + (s.age || "unknown"),
      "Bodyweight: " + (s.bodyweight || "unknown") + " kg",
  ...
  function normalizeIntakeProfile(raw) {
    ...
    return {
      ...
      trainsMultipleLocations: s.trainsMultipleLocations === true,
      secondaryLocationDays: Array.isArray(s.secondaryLocationDays)
        ? s.secondaryLocationDays.filter(function (d) {
            return DAY_KEYS.indexOf(d) >= 0;
          })
        : [],
      secondaryLocationEquipment: String(s.secondaryLocationEquipment || "").slice(0, 600),
      secondaryHeaviestImplementKg:
        parseInt(s.secondaryHeaviestImplementKg, 10) > 0 ? parseInt(s.secondaryHeaviestImplementKg, 10) : 0,
      sessionMinutesByDay: asObj(s.sessionMinutesByDay),
      deloadEveryWeeks: parseInt(s.deloadEveryWeeks, 10) > 0 ? parseInt(s.deloadEveryWeeks, 10) : 0,
      ...
      equipmentList: asObj(s.equipmentList),
      secondaryEquipmentList: asObj(s.secondaryEquipmentList),
  ...
  function athleteProfileForGenerateBlock(profile, opts) {
    ...
    return {
      ...
      trainsMultipleLocations: p.trainsMultipleLocations === true,
      secondaryLocationDays: Array.isArray(p.secondaryLocationDays) ? p.secondaryLocationDays : [],
      secondaryLocationEquipment: String(p.secondaryLocationEquipment || ""),
      secondaryHeaviestImplementKg: parseInt(p.secondaryHeaviestImplementKg, 10) > 0
        ? parseInt(p.secondaryHeaviestImplementKg, 10)
        : 0,
      sessionMinutesByDay: isObj(p.sessionMinutesByDay) ? p.sessionMinutesByDay : {},
      deloadEveryWeeks: parseInt(p.deloadEveryWeeks, 10) > 0 ? parseInt(p.deloadEveryWeeks, 10) : 0,
      ...
      equipmentList: asObj(p.equipmentList),
      /* THE SECOND PLACE'S KIT, WITHOUT WHICH THE CHECK JUDGES SATURDAY BY THE GYM.
         An athlete may train in a fitted gym on weekdays and a garage with two dumbbells on
         Saturday. The questionnaire asks it, the packet sent to the coach carries both lists
         with their days, and athleteCheckCtx (api/personal-coach.js) is built to split the
         week between them — but it looks for THIS field, and this is where it stopped. So
         every day was judged against the first place's inventory: either a sound garage
         session was reported as a violation, or a real one in it was missed. A wrong
         violation also costs money, because it sends a repair pass nobody needed
         (owner, 2026-09-20). */
      secondaryEquipmentList: asObj(p.secondaryEquipmentList),
  ...
  return {
    ...
    GOAL_DEFS: GOAL_DEFS,
    GOAL_SKILL_DEFS: GOAL_SKILL_DEFS,
    MAX_GOALS: MAX_GOALS,
