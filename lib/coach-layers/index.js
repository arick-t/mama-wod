...
const LAYER2_ROOM_STUDIO = require("./layer2-room-studio.js");
const LAYER2_DAYS_INDIVIDUAL = require("./layer2-days-individual.js");
const LAYER2_CONTINUATION = require("./layer2-continuation.js");
 ...
 *
 * A SECOND BUG, found 2026-09-03: the lookbehind above was too strict. Hebrew attaches its
 * prepositions and the definite article to the front of the word — "בסנאץ", "הסנאץ", "במאסל אפ",
 * "המנוע" — so "not preceded by any Hebrew letter" rejected the most natural way an athlete
 * writes a goal. Eight of nine real phrasings missed: "לשפר את הסנאץ שלי" selected no layer at
 * all. The lookbehind now blocks only Hebrew letters that are NOT prefixes, and the suffix
 * lookahead keeps doing the work it was added for — מתח inside מתחיל and מתחרה is still rejected
 * on its right-hand side, prefix or no prefix.
 */
const HEB_PREFIX = "בהוכלמש";
const HEB_NOT_PREFIX = "אגדזחטינסעפצקרתךםןףץ";
function heb(term) {
  return "(?<![" + HEB_NOT_PREFIX + "])" + term + "(?![א-ת])";
}
const HEB = {
  gymnastics: [
    "ג['׳’]ימנסטיק\[א-ת]*", "גימנסטיק\[א-ת]*", "מאסל[ -]?אפ", "עמידת ידיים", "הליכת ידיים",
    "טבעות", "פיסטול\[א-ת]*", "קיפינג", "מתח", "כפיפות מתח", "חבל",
  ],
  weightlifting: ["הרמות אולימפיות", "אולימפי\[א-ת]*", "הנפות", "סנאץ['׳’]?", "קלין", "ג['׳’]רק"],
  endurance: ["אירובי\[א-ת]*", "סיבולת", "מנוע", "ריצה", "חתירה", "אופניים", "סקי", "קרדיו", "אופני אוויר", "לרוץ", "לחתור", "לשחות", "שחייה", "רכיבה"],
...
/* What this block is FOR, as marks rather than prose. The individual intake stopped
   asking for a free line on 2026-09-15, so without this an athlete whose goal is "Engine
   and cardio endurance" would state an intent that nothing here ever reads, and select no
   discipline layer at all. The labels are the text - they are written to be read. */
const PACKET_BLOCK_GOALS = /^BLOCK GOALS: *([^\n]*)$/im;
...
function intentText(profile, studioIntake) {
  ...
  const packetText = textOf(p.fixedIntakePacket);
  const packetGoals = (packetText.match(PACKET_GOALS_SECTION) || [])[1] || "";
  const packetBlockGoals = (packetText.match(PACKET_BLOCK_GOALS) || [])[1] || "";
  return [
    textOf(p.goals),
...
/* Is this a continuation brick? Either the caller says so outright, or the block starts past week
   one. Anything absent or unparseable means "first brick" — the layer must never fire on a genuine
   first block, where a rule about a previous month would be a lie. */
function isContinuationBrick(opts) {
  const o = opts && typeof opts === "object" ? opts : {};
  if (o.continuation === true) return true;
  if (o.blockHandoff) return true;
  const start = parseInt(o.blockStartWeek, 10);
  return start > 1;
}
...
function buildLayerPack(opts) {
  ...
  if (programming) {
    ...
    /* POL-009: brick 2 is not brick 1. Only a continuation pays for it. */
    if (isContinuationBrick(o)) add("layer2-continuation", LAYER2_CONTINUATION);
    /* The rules that differ completely by product: a studio reads what its ROOM is (days, people
       at once, what the place does not do); an individual reads their own days and the limit on
       consecutive ones. Each side reads only its own, instead of both paying for the other's. */
    add(
      o.agent === "studio" ? "layer2-room-studio" : "layer2-days-individual",
      o.agent === "studio" ? LAYER2_ROOM_STUDIO : LAYER2_DAYS_INDIVIDUAL
    );
...
module.exports = {
  ...
  isContinuationBrick: isContinuationBrick,
