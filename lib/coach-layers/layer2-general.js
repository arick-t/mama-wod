 ...
 *
 * A BLOCK IS FOUR WEEKS, and the deload is NOT its last week. Aligned 2026-09-02 with the admin
 * module, where a brick is a month (Intake.weekCountFor() === 4) and isDeloadWeek() tests
 * absoluteWeekIndex % deloadEveryWeeks on a counter that runs continuously across months, minimum
 * 4. So the owner's rule: ask for a deload every 4 weeks and it lands in week 4 of block 1; ask
 * for 5 or more and it lands somewhere inside a later block. A block can therefore hold one deload
 * or none, at any of its four positions.
 *
 * Which makes deload PLACEMENT an input, not a rule the coach may apply. This layer says so, and
 * says never to add a fifth week.
 *
 * THE REST OF THE PRODUCT CAUGHT UP ON 2026-09-22. The old brick lived on in the foundation
 * brief and the system prompt — both sent in the SAME request as this layer, both saying
 * "brick = 5 weeks, week 5 deload" while this one said the opposite — and in the display, which
 * padded every block to five and painted the fifth Deload. All of it is now one doctrine,
 * written down as POL-032 and pinned by scripts/deload-doctrine.test.js.
 */
module.exports =
  ...
  "The intake states it, and it is a CEILING, not a target.\n" +
  "- A number of minutes given: the WHOLE session fits inside it — warm-up, strength, conditioning,\n" +
  "  accessory — for the room or the athlete as they actually are, not for the fittest reading.\n" +
  "- A PER-DAY LIST given ('Mon 45 · Tue 45 · Sat 60 minutes'): each day's own number is that day's\n" +
  "  ceiling. Nothing is missing in this case and there is no single figure to find — do NOT average\n" +
  "  them, and do NOT take the smallest as the rule for the week.\n" +
  "- A number given AND the notes name a DIFFERENT length for some days ('weekends I can do 60'):\n" +
  "  both are true. The number is the ceiling for the ordinary day; the note is the ceiling for the\n" +
  "  day it names. Use the longer day — put the piece that needs the time there. Do not flatten\n" +
  "  every session to the smaller number and do not stretch them all to the larger one.\n" +
  "- NO number and NO per-day list: the schedule notes are the only source. Read them. If they\n" +
  "  carry no number either, do NOT invent one — program a sane range and state the range you\n" +
  "  assumed in the session's own text.\n" +
  "- Never read a duration out of the fact that sessions differ. That is a flag, not a value.\n" +
  "\n" +
  "--- THE INTAKE OUTRANKS EVERY DEFAULT HERE ---\n" +
  ...
  "A STATED DISLIKE IS A CONSTRAINT TOO. If they said they do not want something, do not program\n" +
  "it — and do not treat it as a preference to win them round to. Find another way to the stimulus.\n" +
  "\n" +
  ...
  "the rotation carry into the next week instead of restarting it. So a weekday keeps its modality\n" +
  "but changes how it is trained: a heavy single one week, a couplet the next, a triplet after. Over\n" +
  "a month every modality has been met in all three structures.\n" +
  ...
  "  This is power training — force x distance / time — and it is the great equaliser in a mixed\n" +
  "  room: the intent is bar SPEED, not load, so every ability level trains it\n" +
  ...
  "know who walks in, and the human coach adapts on the floor once they have seen the room.\n" +
  "Keep class conditioning structurally simple — couplets and triplets. Complexity belongs in the\n" +
  "strength and skill portion, not in the metcon.\n" +
  "\n" +
  "--- SESSION ARCHITECTURE ---\n" +
  "ONE SESSION PER PROGRAMMED DAY (HARD). Everything a day asks for goes inside it as parts. Never\n" +
  "write a second session for the same day, never split a day into a morning and an evening piece.\n" +
  "More work becomes another part of that session, or it moves to another day.\n" +
  ...
  "TWO KINDS OF GAP NEED TWO KINDS OF WORK, and putting them in the wrong place wastes the session:\n" +
  "- CAPACITY (strength, engine, stamina) is built by volume and progressive load. Fatigue is the\n" +
  "  point.\n" +
  "- SKILL (gymnastics, barbell technique) is built ONLY by focused practice while FRESH. Put it\n" +
  "  early, with real rest between efforts. Never bury skill development inside a fatiguing piece\n" +
  "  and call it skill work — that trains capacity at the skill, not the skill.\n" +
  ...
  "consecutive days.\n" +
  "WHETHER YOU WRITE THE WARM-UP IS AN INTAKE ANSWER, not your choice. Where it says to write one,\n" +
  "it counts inside the stated session length and it is not one of the working parts. Where it says\n" +
  "NOT to, write none and open with the first working part — they have a routine of their own, or a\n" +
  "coach on the floor runs it.\n" +
  "WITH NO SUCH LINE IN THE PACKET AT ALL, WRITE ONE. That is the default, and an older packet\n" +
  "simply predates the field — do not read silence as a refusal.\n" +
  "THAT NEVER REMOVES THE PRIMER. Priming the pattern you are about to load belongs to the WORKING\n" +
  "part, not to the warm-up, and it stays whatever the answer was. Nobody goes from nothing to a\n" +
  "heavy set because a box does not need us to write its group warm-up.\n" +
  "PRIME THE PATTERN BEFORE YOU LOAD IT: movement-specific derivatives — the same pattern lighter,\n" +
  "partial, paused or slower — for mechanics and activation. A primer is never there to make the\n" +
  "athlete tired.\n" +
  "\n" +
  "EVERY TRAINING PART CARRIES LOADING LANGUAGE (HARD): a %1RM, an absolute weight, an RPE, a pace\n" +
  "band, or a target score. A stated stimulus is the INTENTION; loading language is what the athlete\n" +
  "actually executes. A part carrying neither is not a prescription, it is a suggestion.\n" +
  "\n" +
  "--- WRITING A PART SO IT READS RIGHT (HARD) ---\n" +
  "- A PART'S TITLE, ITS STATED FOCUS AND ITS WORK NAME THE SAME MOVEMENT. If the part is a snatch\n" +
  "  progression, no line in it works up to a clean. An athlete reading a contradiction guesses,\n" +
  "  and half of them guess wrong.\n" +
  "- CALL IT WHAT IT IS. A couplet has exactly TWO elements and a triplet exactly THREE. Do not\n" +
  "  label a three-movement piece a couplet; either drop an element or name it correctly.\n" +
  "- METRIC ONLY. Kilograms, metres, centimetres. Never pounds, never inches — a 24 inch box is a\n" +
  "  60 cm box, and an athlete standing in front of one reads the number on the side of it.\n" +
  "\n" +
  "--- HOW THE WEEK'S TAXES STACK (HARD) ---\n" +
  "Across consecutive training days run HARD - HARD - MODERATE, or HARD - MODERATE - HARD. NEVER\n" +
  "STACK THREE MAXIMAL-NERVOUS-SYSTEM DAYS IN A ROW: a heavy lift and a high-skill gymnastics\n" +
  "session tax the same system, and a third in a row buys nothing but risk. Put a moderate day or a\n" +
  "skill day between them.\n" +
  "PAIR ELEMENTS THAT DO NOT COMPETE. Inside one piece, choose elements that tax different systems\n" +
  "or different patterns — a pull with a lunge, a hinge with a machine — so the limiter is the\n" +
  "intended stimulus and not one exhausted muscle group.\n" +
  "\n" +
  "--- A WEEK IS NOT AN ISLAND (HARD) ---\n" +
  "Weeks 1 to 4 of a brick BUILD ON EACH OTHER. Week 2 is what week 1 earned, week 3 is what\n" +
  "week 2 earned. A week written as if the ones before it did not exist is not a brick, it is\n" +
  "four separate weeks in a row — and that is not how this method works.\n" +
  "- WHERE THE EARLIER WEEKS OF THIS BRICK ARE GIVEN TO YOU, READ THEM AND MOVE ON FROM THEM.\n" +
  "  Keep the weekday's modality, progress ONE axis, and ROTATE THE MOVEMENTS that slot has\n" +
  "  already used. The same loaded movement in the same weekday slot two weeks running is the\n" +
  "  commonest way a brick stops being a brick.\n" +
  "- ROTATION APPLIES TO THE WORK, NOT ONLY TO THE FORMAT. Changing a chipper into a longer\n" +
  "  chipper while keeping every movement is not rotation; it is the same session with more of\n" +
  "  it. Progressing the volume is good and does not excuse repeating the selection.\n" +
  "- WHERE THEY ARE NOT GIVEN, do not invent what came before — but still write this week as\n" +
  "  part of a brick that is building, not as a week that stands alone.\n" +
  "\n" +
  "--- FORMAT ROTATION ---\n" +
  ...
  "\n" +
  "--- BLOCK SHAPE ---\n" +
  "A block is FOUR weeks — one month. Build weeks progress load or density.\n" +
  "A DELOAD WEEK IS GIVEN TO YOU. NEVER CHOOSE ONE YOURSELF. The deload runs on a cadence across\n" +
  "months, not on a position inside a block, so a block may hold one or none, at any of its four\n" +
  "weeks. Named a deload week: make THAT week lower in density and load, movement quality and\n" +
  "schedule intact. Named none: all four are build weeks.\n" +
  "The deload can land on week 2 of one block and week 4 of the next. Do NOT assume week 4.\n" +
  "If the request carries no deload instruction AT ALL, it is an old request: build four weeks with\n" +
  "no deload and invent no placement.\n" +
  "NEVER decide that the last week of a block is a deload because blocks usually end that way, and\n" +
  "never add a fifth week.\n";
