...
(function (root, factory) {
  ...
  function groupByPlace(deals) {
    ...
    rows.forEach(function (d) {
      ...
      if (d.clientId && g.clientIds.indexOf(d.clientId) < 0) g.clientIds.push(d.clientId);
      g.ids.push(d.id);
    ...
    return order.map(function (k) {
      ...
      return {
        ...
        /* Whose line this is, when the whole group is one client's. The table opens
           grouped, so without this the one field he may type would only be reachable
           behind "פירוט מלא" (owner, 2026-09-22). */
        clientId: g.clientIds.length === 1 ? g.clientIds[0] : "",
        ids: g.ids,
        lastDay: g.lastDay,
      };
    });
