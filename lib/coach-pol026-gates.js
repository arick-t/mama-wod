...
/** Max consecutive rest days in a sorted ISO list. */
function maxConsecutiveRestStreak(restIsos) {
  const sorted = uniqSortedIsos(restIsos || []);
  if (!sorted.length) return 0;
  let max = 1;
  let cur = 1;
  for (let i = 1; i < sorted.length; i++) {
    if (daysBetweenIso(sorted[i - 1], sorted[i]) === 1) cur++;
    else {
      if (cur > max) max = cur;
      cur = 1;
    }
...
function longestRestStreakSpan(restIsos) {
  const sorted = uniqSortedIsos(restIsos || []);
  if (!sorted.length) return null;
  let best = { len: 1, startIso: sorted[0], endIso: sorted[0] };
  let curStart = sorted[0];
  let curLen = 1;
  for (let i = 1; i < sorted.length; i++) {
    if (daysBetweenIso(sorted[i - 1], sorted[i]) === 1) curLen++;
    else {
      if (curLen > best.len) best = { len: curLen, startIso: curStart, endIso: sorted[i - 1] };
      curStart = sorted[i];
      curLen = 1;
    }
  }
  if (curLen > best.len) best = { len: curLen, startIso: curStart, endIso: sorted[sorted.length - 1] };
  return best;
}

function mergeRestIsoSets(existing, added) {
  return uniqSortedIsos([].concat(existing || [], added || []));
}

function expandIsoRange(startIso, endIso) {
  const out = [];
  let cur = String(startIso || "").slice(0, 10);
...
/** Returns streak info when SET would lengthen max consecutive rest vs calendar baseline. */
function detectRestStreakIncrease(existingRestIsos, addedRestIsos) {
  const beforeMax = maxConsecutiveRestStreak(existingRestIsos);
  const merged = mergeRestIsoSets(existingRestIsos, addedRestIsos);
  const afterMax = maxConsecutiveRestStreak(merged);
  if (afterMax <= beforeMax) return null;
  const span = longestRestStreakSpan(merged);
  return {
    beforeMax: beforeMax,
    afterMax: afterMax,
    startIso: span ? span.startIso : null,
    endIso: span ? span.endIso : null,
    streakLen: span ? span.len : afterMax,
  };
}

function isSetRestIntent(intent) {
  return !!(intent && intent.restDays && intent.restDays.length && !intent.replaceNextRest);
}

/**
 * Budget POL-026 phase: set_auto | streak_choice | replace_confirm | default_confirm
 * @returns {{ phase: string, autoApply: boolean, intent: object, setRest?: string[], streakInfo?: object }}
 */
function evaluateBrickSchedulePhase(intent, existingRestIsos) {
  const existing = uniqSortedIsos(existingRestIsos || []);
  if (intent && intent.replaceNextRest) {
    return { phase: "replace_confirm", autoApply: false, intent: intent };
  }
  if (isSetRestIntent(intent)) {
    const setRest = uniqSortedIsos(intent.restDays);
    const streakInfo = detectRestStreakIncrease(existing, setRest);
    if (!streakInfo) {
      return { phase: "set_auto", autoApply: true, intent: intent, setRest: setRest };
    }
    return {
      phase: "streak_choice",
      autoApply: false,
  ...
  return { phase: "default_confirm", autoApply: false, intent: intent };
}

const REST_STREAK_KEEP_RE = /^בסדר\s*ככה[.!\s]*$/i;
const REST_STREAK_MOVE_RE = /^תזיז\s*מנוחה[.!\s]*$/i;

function isRestStreakKeepChoice(text) {
  return REST_STREAK_KEEP_RE.test(String(text || "").trim());
}

function isRestStreakMoveChoice(text) {
  return REST_STREAK_MOVE_RE.test(String(text || "").trim());
}

function isRestStreakApplyChoice(text) {
  return isRestStreakKeepChoice(text) || isRestStreakMoveChoice(text);
}

function buildRestStreakChoiceMessage(streakInfo, opts) {
  opts = opts || {};
  const hebrew = opts.hebrew !== false;
  const info = streakInfo || {};
  const len = info.streakLen || info.afterMax || 2;
  const startLabel = info.startIso
    ? hebrew
      ? isoToHebDayLabel(info.startIso)
      : isoToEnDayLabel(info.startIso)
    : "";
  const endLabel = info.endIso
    ? hebrew
      ? isoToHebDayLabel(info.endIso)
      : isoToEnDayLabel(info.endIso)
    : "";
  if (hebrew) {
    const range =
      startLabel && endLabel && startLabel !== endLabel
        ? startLabel + "–" + endLabel
        : startLabel || String(len);
    return "אחרי השינוי יצאו " + String(len) + " ימי מנוחה ברצף (" + range + "). מה עושים?";
  }
  const rangeEn =
    startLabel && endLabel && startLabel !== endLabel ? startLabel + "–" + endLabel : startLabel || String(len);
  return "After the change there are " + String(len) + " rest days in a row (" + rangeEn + "). What next?";
}

/** Break an extended rest streak after athlete chooses «תזיז מנוחה» (SET path). */
function planRestStreakShift(existingRestIsos, setRestIsos) {
  const existing = uniqSortedIsos(existingRestIsos || []);
  const setRest = uniqSortedIsos(setRestIsos || []);
  const merged = mergeRestIsoSets(existing, setRest);
  const beforeMax = maxConsecutiveRestStreak(existing);
  const span = longestRestStreakSpan(merged);
  if (!span || span.len <= beforeMax) return { clearRest: [] };
  const streakIsos = expandIsoRange(span.startIso, span.endIso);
  const setMap = {};
  for (let i = 0; i < setRest.length; i++) setMap[setRest[i]] = true;
  const existingMap = {};
  for (let j = 0; j < existing.length; j++) existingMap[existing[j]] = true;
  let clearIso = null;
  for (let k = 0; k < streakIsos.length; k++) {
    const iso = streakIsos[k];
    if (existingMap[iso] && !setMap[iso]) {
      clearIso = iso;
      break;
    }
  }
  if (!clearIso) {
    for (let m = 0; m < streakIsos.length; m++) {
      if (!setMap[streakIsos[m]]) {
        clearIso = streakIsos[m];
        break;
      }
    }
  }
  if (!clearIso && streakIsos.length) clearIso = streakIsos[0];
  return { clearRest: clearIso ? [clearIso] : [] };
}
...
function isScheduleApplyIntent(text) {
  ...
  if (!t) return false;
  if (isRestStreakApplyChoice(t)) return true;
...
function enforcePol026BrickChatResponse(text, opts) {
  ...
  if (opts && opts.localApplied === true) return BRICK_SCHEDULE_POST_APPLY_MSG;
  const phase = opts && opts.phase;
  const streakInfo = opts && opts.streakInfo;
  if (phase === "streak_choice" && streakInfo) {
    const noteForHe = opts && (opts.scheduleNote || opts.note);
    return buildRestStreakChoiceMessage(streakInfo, {
      hebrew: noteForHe ? noteLooksHebrew(noteForHe) : true,
    });
  }
  ...
  if (note) {
    ...
    const swapPlan =
      opts && opts.swapPlan
        ? opts.swapPlan
        : intent.replaceNextRest
          ? planRestDaySwap(intent, opts && opts.existingRestIsos ? opts.existingRestIsos : [])
          : null;
    if (phase === "set_auto") return BRICK_SCHEDULE_POST_APPLY_MSG;
    return buildBrickScheduleConfirmMessage(intent, {
      note: note,
      todayIso: todayIso,
      swapPlan: swapPlan,
    });
