...
function parseBrickScheduleIntent(noteText, opts) {
  ...
  const replaceNextRest =
    /המנוחה\s*(ה)?באה|יום\s*המנוחה\s*(שלאחריו|הבא)|לאחר\s*מכן\s*.{0,24}מנוחה|next\s*rest\s*day|the\s*next\s*rest/i.test(
      t
    );
...
function buildBrickScheduleConfirmMessage(intent, opts) {
  ...
  if (hebrew) {
    ...
    if (intentSafe.restDays && intentSafe.restDays.length) {
      ...
      if (dayLabels.length) {
        parts.push(
          (intentSafe.replaceNextRest ? "המנוחה הבאה ב" : "מנוחה ב") + dayLabels.join(" וב")
        );
      }
    }
    if (intentSafe.replaceNextRest) parts.push("בלי שני ימי מנוחה רצופים");
  ...
  if (intentSafe.restDays && intentSafe.restDays.length) {
    ...
    enParts.push((intentSafe.replaceNextRest ? "next rest on " : "rest on ") + labels.join(" and "));
  }
  if (intentSafe.replaceNextRest) enParts.push("no back-to-back rest pile-up");
