...
(function (root, factory) {
   ...
   */
  /* A name the owner gave the day — "אימון תחנות" — belongs to the DAY, not to what is
     written on it, so it survives becoming a rest day and coming back
     (owner, 2026-09-05). */
  function keptTitle(week, dayKey) {
    var day = isPlainObject(week) && isPlainObject(week.days) ? week.days[dayKey] : null;
    var t = day && typeof day.title === "string" ? day.title.trim() : "";
  ...
  function makeRest(week, dayKey) {
    ...
    var keepRest = keptTitle(week, dayKey);
    week.days[dayKey] = keepRest
      ? { parts: restPartsFor(dayKey), title: keepRest }
      : { parts: restPartsFor(dayKey) };
  ...
  function makeSession(week, dayKey, parts, focus) {
    ...
    var keep = keptTitle(week, dayKey);
    week.days[dayKey] = keep ? { parts: list, title: keep } : { parts: list };
