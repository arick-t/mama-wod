...
(function (root, factory) {
   ...
   */
  /**
   * Phrases that CONTAIN a blocking keyword and yet need nothing but floor.
   *
   * A shuttle run is not running. A studio on the third floor of an office block has no
   * route and never will, and it can still run 10 metres out and back all day — so
   * "shuttle run" matching RUN meant the checker blocked a movement that needs nothing,
   * and the inventory told the coach not to write it in the first place. That is the
   * checker removing good programming, which is worse than the bug it was built to fix
   * (owner, 2026-09-15).
   *
   * Deliberately short and unambiguous. "Out and back" is NOT here: a kilometre out and
   * back is a road run, and suppressing it would hide a real violation.
   */
  var FLOOR_PHRASES = [
    "shuttle run",
    "shuttle runs",
  ...
  function requiredFor(text) {
    ...
    /* The same positional idea, one step earlier: a stretch of text that is a floor phrase
       requires nothing, so every keyword inside it is dropped. */
    var floors = [];
    FLOOR_PHRASES.forEach(function (ph) {
      var from = 0;
      var at;
      while ((at = t.indexOf(ph, from)) !== -1) {
        floors.push({ at: at, end: at + ph.length });
        from = at + 1;
      }
    });
    ...
    var kept = hits.filter(function (h) {
      if (floors.some(function (f) {
        return f.at <= h.at && f.end >= h.end;
      })) {
        return false;
      }
  ...
  function inventoryText(list, opts) {
    ...
    var out = [
      ...
      "SHUTTLE RUNS ARE FLOOR WORK, NOT RUNNING. Short shuttles - 10-15 m out and back - need",
      "nothing but floor and are available in EVERY room, including one with no running route.",
      "\"RUN NOT AVAILABLE\" forbids a route, never a shuttle.",
    ].concat(rows);
