...
(function (root, factory) {
  ...
  var TERMS = [
    ...
    /* --- press and push -------------------------------------------------- */
    { en: "bench press", he: "לחיצת חזה בשכיבה" },
    ...
    /* --- monostructural --------------------------------------------------- */
    { en: "shuttle run", he: "ספרינטים" },
    { en: "shuttle sprint", he: "ספרינטים" },
    { en: "run", he: "ריצה" },
    { en: "running", he: "ריצה" },
    { en: "row", he: "חתירה" },
    { en: "rowing", he: "חתירה" },
    ...
    { en: "burpee", he: "סמוך-קום" },
    { en: "bearpee", he: "סמוך-קום" },
  ...
  /**
   * THE FORMAT LINE, and the units inside every line.
   *
   * The owner's own corrections, kept as he wrote them: he does not translate a format,
   * he EXPLAINS it. "AMRAP 16 minutes" became "16 דקות - מקסימום עבודה" and "For Time -
   * 2 rounds" became "2 סיבובים מהר ככל הניתן". A dictionary cannot reorder a sentence,
   * so the shapes that actually recur are matched whole and rewritten
   * (owner, 2026-09-20; the corrections are in docs/oded/corrections.json).
   *
   * Applied to the FORMAT line only. A work line never goes through them.
   */
  var FORMAT_RULES = [
    { re: /^\s*amrap\s*(?:in\s*)?(\d+)\s*(?:min|mins|minutes)\s*:?\s*$/i, he: "$1 דקות - מקסימום עבודה" },
    { re: /^\s*(\d+)\s*(?:min|mins|minutes)\s*amrap\s*:?\s*$/i, he: "$1 דקות - מקסימום עבודה" },
    { re: /^\s*emom\s*(?:in\s*)?(\d+)\s*(?:min|mins|minutes)\s*:?\s*$/i, he: "$1 דקות - יציאה לתרגיל כל דקה" },
    { re: /^\s*for\s*time\s*[-–]\s*(\d+)\s*rounds?\s*:?\s*$/i, he: "$1 סיבובים - מהר ככל הניתן" },
    { re: /^\s*(\d+)\s*rounds?\s*for\s*time\s*:?\s*$/i, he: "$1 סיבובים - מהר ככל הניתן" },
    { re: /^\s*for\s*time\s*:?\s*$/i, he: "מהר ככל הניתן" },
    { re: /^\s*(\d+)\s*sets?\s*for\s*quality\s*\(\s*climbing\s*load\s*\)\s*:?\s*$/i, he: "$1 סטים (משקל עולה)" },
    { re: /^\s*(\d+)\s*sets?\s*for\s*quality\s*\(([^)]*)\)\s*:?\s*$/i, he: "$1 סטים ($2)" },
    { re: /^\s*(\d+)\s*sets?\s*for\s*quality\s*:?\s*$/i, he: "$1 סטים" },
    { re: /^\s*(\d+)\s*rounds?\s*\(\s*work\s*([0-9:]+)\s*\/\s*rest\s*([0-9:]+)\s*\)\s*:?\s*$/i,
      he: "$1 סטים (עבודה $2 / הפסקה $3)" },
  ];

  /**
   * Units, applied to every line once the movements are in Hebrew. The owner left some
   * in English himself, so these are the ones he consistently turned over.
   */
  /** Numbers and their units. Safe anywhere: they cannot collide with a word. */
  var UNITS = [
    { re: /(\d+)\s*minutes/gi, he: "$1 דקות" },
    { re: /(\d+)\s*mins?(?![a-z])/gi, he: "$1 דקות" },
    { re: /(\d+)\s*seconds/gi, he: "$1 שניות" },
    { re: /(\d+)\s*secs?(?![a-z])/gi, he: "$1 שניות" },
    { re: /(\d+)\s*meters?(?![a-z])/gi, he: "$1 מטר" },
    { re: /(\d+)\s*reps?(?![a-z])/gi, he: "$1 חזרות" },
    { re: /(\d+)\s*rounds?(?![a-z])/gi, he: "$1 סיבובים" },
    { re: /(\d+)\s*sets?(?![a-z])/gi, he: "$1 סטים" },
    { re: /(\d+)\s*cal(?![a-z])/gi, he: "$1 קלוריות" },
  ];

  /**
   * Words that mean something precise on a FORMAT line and something ordinary in prose.
   *
   * "off" is a rest interval in "30 sec on 30 sec off" and a preposition in "neutral
   * spine off floor" — turning the second one into "הפסקה" is how a coaching cue became
   * nonsense. They are applied to the format line alone (owner, 2026-09-20).
   */
  var FORMAT_WORDS = [
    { re: /climbing load/gi, he: "משקל עולה" },
    { re: /(?<![a-z])work(?![a-z])/gi, he: "עבודה" },
    { re: /(?<![a-z])rest(?![a-z])/gi, he: "הפסקה" },
    { re: /(?<![a-z])max(?![a-z])/gi, he: "מקסימום" },
    { re: /(?<![a-z])on(?![a-z])/gi, he: "עבודה" },
    { re: /(?<![a-z])off(?![a-z])/gi, he: "הפסקה" },
    { re: /(?<![a-z])each(?![a-z])/gi, he: "כל" },
  ];

  /**
   * THE WORDS A PART IS NAMED WITH.
   *
   * A session's sub-heading is a movement plus a qualifier — "Back Squat Heavy Volume",
   * "Deadlift Strength", "Engine Aerobic Intervals". The movement half already turns
   * over; these are the other half, and without them the heading stayed English over a
   * Hebrew session (owner, 2026-09-20).
   *
   * Small on purpose: a coach names parts from a short, repeating vocabulary. "מטקון" is
   * his own word for a conditioning piece.
   */
  var TITLE_WORDS = [
    { en: "heavy volume", he: "נפח כבד" },
    { en: "conditioning couplet", he: "מטקון" },
    { en: "chipper conditioning", he: "מטקון" },
    { en: "extended conditioning", he: "מטקון ארוך" },
    { en: "conditioning", he: "מטקון" },
    { en: "metcon", he: "מטקון" },
    { en: "couplet", he: "מטקון" },
    { en: "chipper", he: "מטקון" },
    { en: "aerobic engine", he: "מנוע אירובי" },
    { en: "engine aerobic", he: "מנוע אירובי" },
    { en: "interval rounds", he: "סיבובי אינטרוולים" },
    { en: "intervals", he: "אינטרוולים" },
    { en: "interval", he: "אינטרוול" },
    { en: "progression", he: "פרוגרסיה" },
    { en: "strength", he: "כוח" },
    { en: "power", he: "כוח מתפרץ" },
    { en: "endurance", he: "סיבולת" },
    { en: "capacity", he: "קיבולת" },
    { en: "stamina", he: "סבולת" },
    { en: "engine", he: "מנוע" },
    { en: "aerobic", he: "אירובי" },
    { en: "anaerobic", he: "אנאירובי" },
    { en: "volume", he: "נפח" },
    { en: "heavy", he: "כבד" },
    { en: "extended", he: "ארוך" },
    { en: "lower body", he: "פלג גוף תחתון" },
    { en: "upper body", he: "פלג גוף עליון" },
    { en: "core", he: "ליבה" },
    { en: "midline", he: "ליבה" },
    { en: "accessory", he: "משלים" },
    { en: "skill", he: "מיומנות" },
    { en: "technique", he: "טכניקה" },
    { en: "warm-up", he: "חימום" },
    { en: "warm up", he: "חימום" },
    { en: "cool-down", he: "שחרור" },
    { en: "cool down", he: "שחרור" },
    { en: "finisher", he: "פיניש" },
    { en: "pacing", he: "קצב" },
    { en: "quality", he: "איכות" },
    { en: "high intensity", he: "עצימות גבוהה" },
    { en: "for time", he: "לזמן" },
    { en: "focus", he: "פוקוס" },
    { en: "hiking", he: "מסע" },
  ];
  ...
  /* Longest first, through the same matcher the movements use — hand-rolling a second
     one is how the first attempt matched nothing at all. */
  var TITLE_PATTERNS = TITLE_WORDS.slice()
    .sort(function (a, b) {
      return b.en.length - a.en.length;
    })
    .map(function (t) {
      return { he: t.he, re: patternFor(t.en) };
    });
  ...
  function hebrewOnly(text) {
    ...
    /* Units are turned over even when the line names no movement — "10 min - 30 sec on
       30 sec off" is a whole format line with nothing in it to look up. */
    if (!out.length) return units(line).replace(/\s{2,}/g, " ").trim();
    out.sort(function (a, b) {
      return a.at - b.at;
    });
    var built = "";
    ...
    return units(built + line.slice(cursor)).replace(/\s{2,}/g, " ").trim();
  }

  /**
   * A PART'S NAME, in Hebrew.
   *
   * The movements first, then the words a part is named with. A heading left in English
   * over a Hebrew session is the one thing that still looked half-done
   * (owner, 2026-09-20).
   */
  function titleLine(text) {
    var line = hebrewOnly(text);
    for (var i = 0; i < TITLE_PATTERNS.length; i++) {
      var he = TITLE_PATTERNS[i].he;
      TITLE_PATTERNS[i].re.lastIndex = 0;
      line = line.replace(TITLE_PATTERNS[i].re, function (m, pre) {
        return pre + he;
      });
    }
  ...
  /**
   * A FORMAT line, in Hebrew — explained rather than translated.
   *
   * The shapes that recur are rewritten whole; anything else falls through to the same
   * pass every other line gets.
   */
  function formatLine(text) {
    var line = String(text == null ? "" : text).trim();
    if (!line) return "";
    for (var i = 0; i < FORMAT_RULES.length; i++) {
      var m = FORMAT_RULES[i].re.exec(line);
      if (m) {
        return FORMAT_RULES[i].he.replace(/\$(\d)/g, function (_, n) {
          return m[Number(n)] || "";
        });
      }
    }
    return units(hebrewOnly(line), true).replace(/\s{2,}/g, " ").trim();
  }

  /** Units, after the movements. Kept apart so a work line and a note get the same pass. */
  function units(text, alsoFormatWords) {
    var line = String(text == null ? "" : text);
    for (var i = 0; i < UNITS.length; i++) {
      line = line.replace(UNITS[i].re, UNITS[i].he);
    }
    if (alsoFormatWords === true) {
      for (var j = 0; j < FORMAT_WORDS.length; j++) {
        line = line.replace(FORMAT_WORDS[j].re, FORMAT_WORDS[j].he);
      }
    }
    return line;
  }
