...
/**
 * Is there a real session written in this day?
 *
 * A part that only says "Rest" is a marker, not programming. Anything else with a
 * line in it, or with a name somebody typed, is a session.
 */
function pprogPartsHoldSession(parts) {
  if (!Array.isArray(parts) || !parts.length) return false;
  for (var i = 0; i < parts.length; i++) {
    var p = parts[i] || {};
    var lines = Array.isArray(p.lines) ? p.lines : [];
    var blob = (String(p.title || "") + " " + lines.join(" ")).toLowerCase().trim();
    if (!blob) continue;
    if (/מנוחה/.test(String(p.title || "") + " " + lines.join(" "))) continue;
    if (/^(rest(\s*day)?|off(\s*day)?)\b/.test(blob)) continue;
    if (String(p.id || "").indexOf("overview-stub") >= 0) continue;
    if (/still loading/i.test(blob)) continue;
    return true;
  }
  return false;
}
...
function isPprogRestDay(dayKey, dayData, week) {
  ...
  if (isPprogRestFocus(focus)) {
    /* THE OVERVIEW DOES NOT OUTRANK A WRITTEN SESSION.
       It used to: a day whose weekly overview row said "Rest" rendered as a rest day
       even when a full workout was written in it — and because a rest card hides the
       parts, there was then no way to reach the workout and no way to take the rest
       day off. The owner hit exactly that on a real client, on his phone, on the last
       day of a block (owner, 2026-09-09): "יום המנוחה לא מסומן בצ'קבוקס שלו וגם אי
       אפשר למחוק או להחליף אותו".
       lib/day-rest-toggle.js has always read it this way ("Trust the parts — that is
       the bug this module exists to prevent"); the renderer now agrees with it, which
       also means the stuck day comes back on its own the moment the page reloads. */
    if (pprogPartsHoldSession(parts)) return false;
    return true;
  }
...
function pprogIsDeloadWeek(week, weekIndex0, block) {
  ...
  /* NO PRESUMPTION. A deload is a deload because the coach wrote it as one, and for no
     other reason. The fifth week used to be assumed to be one - which is how a week nobody
     planned came to be labelled a deload, and how a block with a deload on week 3 ended up
     with two (owner, 2026-09-22, POL-032). */
  return false;
}
...
function normalizePprogWeek(week, prevWeek, weekStartOverride, weekMeta) {
  ...
  for (var i = 0; i < PPROG_DAY_KEYS.length; i++) {
    ...
    var athleteDayAt =
      (day && day.athleteDayUpdatedAt) || (prev && prev.athleteDayUpdatedAt) || "";
    if (athleteDayAt) dayOut.athleteDayUpdatedAt = String(athleteDayAt).slice(0, 40);
    var coachNotice =
      (day && day.coachUpdatedNotice) || (prev && prev.coachUpdatedNotice) || "";
    if (coachNotice) dayOut.coachUpdatedNotice = String(coachNotice).slice(0, 120);
    var coachNoticeAt = (day && day.coachUpdatedAt) || (prev && prev.coachUpdatedAt) || "";
    if (coachNoticeAt) dayOut.coachUpdatedAt = String(coachNoticeAt).slice(0, 40);
...
function normalizePprogBlock(block, prevBlock) {
  ...
  var weeks = [];
  /* AS MANY WEEKS AS THERE ARE. NOT FIVE.
     This used to run to five whatever it was given, and it did two things nobody asked
     for: it INVENTED an empty fifth week - labelled "deload", for an athlete who may have
     asked for no deload at all - and it silently DROPPED a sixth. A brick is four weeks of
     training (POL-032); a deload is a cadence the intake chooses, and the week it falls on
     is named in the request. A block already saved with five weeks still normalises to five,
     because the rule is "do not add one", never "cut to four" (owner, 2026-09-22). */
  var howMany = Math.max(srcWeeks.length, prevWeeks.length);
  ...
  return {
    summaryLine: (block && block.summaryLine) || (prevBlock && prevBlock.summaryLine) || "4-week training block",
  ...
  return {
    ...
    partsHoldSession: pprogPartsHoldSession,
