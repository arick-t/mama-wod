...
(function (root, factory) {
  ...
  function copyPart(p, id) {
    ...
    /* The shape of the part — see lib/pprog-display.js. Without it a copy is read
       differently from the day it was copied from (owner, 2026-09-05). */
    const formatLine = parseInt(src.formatLine, 10);
    ...
    /* The hidden English travels with the part. It is the only language the mechanical
       check can read, so a Hebrew part copied to another day without it lands there
       unchecked (owner, 2026-09-20). */
    if (Array.isArray(src.linesEn) && src.linesEn.length) {
      out.linesEn = src.linesEn.map(function (l) {
        return str(l, 600);
      });
    }
    if (src.titleEn) out.titleEn = str(src.titleEn, 200);
    return out;
  }
  ...
  function weekPayload(week) {
    ...
    /* How many sessions this week holds, when the week says so itself. A block sold as
       four a week keeps its four wherever it is planted, even at a client who trains
       seven (owner, 2026-09-08). */
    const sessions = parseInt(src.sessions, 10);
    if (sessions >= 1 && sessions <= 7) out.sessions = sessions;
  ...
  function pasteWeek(weeks, wi, payload) {
    ...
    const sessions = parseInt(src.sessions, 10);
    if (sessions >= 1 && sessions <= 7) week.sessions = sessions;
    else delete week.sessions;
    return { ok: true };
  }
