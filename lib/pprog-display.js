...
(function (root, factory) {
   ...
   */
  function classifyPartLines(rawLines, authoredNotes) {
    ...
    var trailingNotes = [];
    var formatIdx = -1;
    /* He said how many. Take them, and classify what is left as usual. */
  ...
  function renderDayPartsHtml(parts, dayData, opts) {
    ...
    for (var pi = 0; pi < parts.length; pi++) {
      ...
      var classified = classify(part.lines, part.noteLines) || { notes: [], format: "", work: [], trailingNotes: [] };
      ...
      for (var li = 0; li < work.length; li++) {
        ...
        html += '<li dir="auto">' + esc(work[li]) + "</li>";
      }
      html += "</ul>";
  ...
  function draftFromDayData(dayData, restDay) {
    ...
    for (i = 0; i < parts.length; i++) {
      ...
      var c = classifyPartLines(p.lines, p.noteLines) || { notes: [], format: "", work: [], trailingNotes: [] };
  ...
  function partsFromDraft(draft) {
    ...
    for (var i = 0; i < src.length; i++) {
      ...
      /* How many of the lines above are notes he wrote as notes. Written down rather
         than guessed later — see classifyPartLines (owner, 2026-09-05). */
      var noteCount = 0;
      for (j = 0; j < notes.length; j++) {
  ...
  function renderAdminDayEditHtml(opts, restVisual) {
    ...
    for (pi = 0; pi < parts.length; pi++) {
      ...
      for (li = 0; li < work.length; li++) {
        html +=
          '<li class="pprog-edit-work-row"><input type="text" dir="auto" value="' +
          esc(work[li] || "") +
  ...
  function renderDayCardHtml(block, week, activeWi, day, opts) {
    ...
    var titleEditor =
      editing && opts.allowTitleEdit === true
        ? '<input type="text" class="pprog-day-title-in" data-day-title="1" maxlength="80" dir="auto"' +
          ' placeholder="' + esc(opts.dayTitlePlaceholder || "שם ליום…") + '" value="' + esc(dayTitle) + '"' +
          ' aria-label="שם היום">'
        : "";
    ...
    /* Always beside the pencil, named or not — he opened a day he had never touched and
       ...
       read-only render), it stays the heading instead, or the day would lose its name
       entirely. */
    ...
    html +=
      ...
      (titleEditor ? titleEditor : dayTitle ? esc(dayTitle) : allowEdit ? "" : esc(dateLabel)) +
      (loggedExtra
  ...
  function dayShareText(block, weekIndex0, dayKey, opts) {
    ...
    for (var pi = 0; pi < parts.length; pi++) {
      var part = parts[pi] || {};
      var c = classifyPartLines(part.lines, part.noteLines) || { notes: [], format: "", work: [], trailingNotes: [] };
