...
(function (root, factory) {
  ...
  function renderCalHtml(block, activeWi, activeDay, opts) {
    ...
    if (calMode === "month") {
      ...
      html += "</div>";
      html +=
        '<div class="pprog-cal-block-hint">Full ' + weekCount +
        "-week brick · tap a date · ‹ › moves active week</div>";
    } else {
      var activeStart =
  ...
  function renderDayCardHtml(block, week, activeWi, day, opts) {
    ...
    var shareBtn = "";
    if (showActions && !readOnly && !allowEdit) {
      shareBtn =
    ...
    var dayModified = !!(dayData && dayData.modified);
