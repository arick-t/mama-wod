...
(function (root, factory) {
  ...
  function isDeloadWeek(week, weekIndex0, block) {
    ...
    /* NO FIFTH-WEEK PRESUMPTION. A week is a deload because the coach wrote it as one.
       This drew a fifth row and painted it "Deload" on a four-week brick whose athlete had
       asked for no deload at all — the same assumption that lived in the normaliser, kept
       here as well, so fixing one changed nothing on screen (owner, 2026-09-22, POL-032).
       The minimum of four rows above is the shape of the month while its weeks are still
       landing; it is not a claim that a fifth week exists. */
    return false;
  }
  ...
  /** "Part A", "Part B" — what the card puts there before anyone writes anything. */
  function isGeneratedPartTitle(title) {
    return /^part\s+[a-z]$/i.test(String(title == null ? "" : title).trim());
  }
  ...
  function dayHasRealTrainingParts(dayData) {
    ...
    for (pi = 0; pi < parts.length; pi++) {
      var p = parts[pi];
      /* A part he NAMED is a part, even before a line is written under it: "Part A —
         גב" is the session he is building. Treating it as nothing replaced his own
         heading with "Full session details still loading…", which is the placeholder
         for a day the coach's brain has not filled yet — and his day was not that
         (owner, 2026-09-08). */
      if (p && !isGeneratedPartTitle(p.title) && String((p && p.title) || "").trim()) return true;
  ...
  /**
   * The name of a part, with the "Part A" out of it.
   *
   * Renaming a part used to be impossible to finish: the heading was built as
   * "Part " + letter + " — " + whatever he typed, so the words PART A stayed on the
   * card however he renamed it (owner, 2026-09-09: "שינוי שם == שינוי מלא של שם
   * החלק"). Stored titles the coach's brain wrote carry the prefix inside the text
   * itself ("Part B - bench press Progression"), so it is taken off HERE, at the one
   * place both the card and the edit field read — cleaning the stored data instead
   * would rewrite months of programming to fix a heading.
   */
  function cleanPartTitle(rawTitle) {
    var t = String(rawTitle == null ? "" : rawTitle).trim();
    /* "Part A", "Part A — Strength", "PART B - bench press", "part c: metcon" */
    t = t.replace(/^part\s+[a-z]\b\s*(?:[—–:.-]\s*)?/i, "");
    /* And a part he numbered himself, so his own "1. חימום" is not doubled. */
    t = t.replace(/^([0-9]{1,2})\s*(?:[—–:.)-]\s*)/, "");
    return t.trim();
  }

  /**
   * What the card puts at the top of a part: its POSITION, then its name.
   *
   * The number is not stored anywhere — it is where the part sits, counted from the
   * top — so moving a part up or down renumbers everything by itself, which is what
   * he asked for (owner, 2026-09-09: "1 מיד למעלה וכן הלאה").
   */
  function formatPartHeading(partIndex, rawTitle) {
    var n = (partIndex | 0) + 1;
    var t = cleanPartTitle(rawTitle);
  ...
  function renderDayPartsHtml(parts, dayData, opts) {
    ...
    for (var pi = 0; pi < parts.length; pi++) {
      ...
      /* THE NUMBER IS ITS OWN ELEMENT, and it comes first.
         As one string it fell wherever the bidi algorithm put it — a Hebrew name threw
         it to the far left of the heading, so a column of parts had its numbers on the
         wrong side and out of line (owner, 2026-09-09, with the picture). Split in two,
         the number leads in the PAGE's direction: on the right through the whole of his
         Hebrew back office, on the left on the client's English page — one side per
         page rather than one side per part. The name still decides its own direction,
         so "Back Squat" and "אימון תחנות" both read correctly inside it. */
      var headName = headingFn === formatPartHeading ? cleanPartTitle(part.title) : "";
      var headNum = headingFn === formatPartHeading ? String(pi + 1) : "";
      ...
      /* Which part this is, so the page can copy it or plant one under it — the row is
         where the right-click lands (owner, 2026-09-05). */
      html += '<div class="section-title-row" data-part="' + pi + '">';
      html += headNum
        ? '<div class="section-title has-part-n">' +
          '<span class="pprog-part-n" aria-hidden="true">' + esc(headNum) + "</span>" +
          '<span class="pprog-part-name" dir="auto">' + esc(say(headName, "title")) + "</span>" +
          (modified ? '<span class="pprog-modified-flag">MODIFIED</span>' : "") +
          "</div>"
        : '<div class="section-title" dir="auto">' +
          esc(say(heading, "title")) +
          (modified ? '<span class="pprog-modified-flag">MODIFIED</span>' : "") +
          "</div>";
      html += "</div>";
      var ni;
      ...
      if (classified.format) {
        html += '<div class="pprog-part-format" dir="auto">' + esc(say(classified.format, "format")) + "</div>";
      }
      ...
      for (var li = 0; li < work.length; li++) {
        ...
        html +=
          ...
          (numbered
            ...
            : "") +
          /* A NUMBERED line wraps its text in a box of its own, carrying the same
             dir="auto". On a phone that box becomes the second half of a two-column row
             so the circles stand in a straight line down the left edge whatever
             language the line is in — see the media query in admin.html
             (owner, 2026-09-05, on an iPhone). A line with no number is left exactly as
             it was: one text node, no box, nothing to re-lay-out. */
          (numbered
            ? '<span class="pprog-li-text" dir="auto">' + esc(say(work[li])) + "</span>"
            : esc(say(work[li]))) +
          "</li>";
      }
      html += "</ul>";
  ...
  /* ── Hebrew, beside the English ─────────────────────────────────────────
   * One switch, set by the page from the programme's own field, and one pass that every
   * DISPLAYED string goes through. The edit inputs below are deliberately not routed
   * through it: what he types is what is stored, and a box that shows something other
   * than what it will save is a trap (owner, 2026-09-16).
   * The English never leaves the line — see lib/movement-he.js for why that is a safety
   * rule and not a style choice. */
  var language = "en";

  function setLanguage(lang) {
    language = String(lang || "") === "he" ? "he" : "en";
    return language;
  }
  ...
  /**
   * A string on its way to the screen. Untouched unless Hebrew is on.
   *
   * HEBREW MEANS HEBREW — no English left in brackets. The owner read the bilingual
   * lines and called it noise: "נתחיל ככה, אם נראה שזה לא מסתדר אנחנו נייצר משהו על
   * האמצע" (2026-09-20). The English is not lost — it sits in the part's `linesEn`,
   * which is what every check reads.
   *
   * A FORMAT line is explained rather than translated, in his own phrasing: "AMRAP 16
   * minutes" is "16 דקות - מקסימום עבודה" and not "AMRAP 16 דקות".
   *
   * NOTES AND TITLES ARE NOT ROUTED THROUGH HERE. They are prose — a cue, an intent, a
   * session's name — and a dictionary turning single words over inside a sentence
   * produces something worse than English: "maintain neutral spine off floor" came back
   * as "neutral spine הפסקה floor". Prose is translated once, by the coach, when the
   * month is delivered; this switch is for the parts a table can do exactly.
   */
  function say(text, kind) {
    if (language !== "he") return text;
    /* `root` belongs to the UMD wrapper, not to this factory — it was never in scope
       here, so the lookup failed silently and every line came back untranslated. In Node
       the require below rescued it and the tests passed; in a browser there is no
       require, so nothing happened at all and the switch looked dead (owner, 2026-09-20,
       after a hard refresh changed nothing). */
    var g = typeof self !== "undefined" ? self : typeof globalThis !== "undefined" ? globalThis : null;
    var M = (g && g.MovementHe) || null;
    if (!M && typeof require === "function") {
      try {
        M = require("./movement-he.js");
      } catch (e) {
        M = null;
      }
    }
    if (!M) return text;
    if (kind === "format" && typeof M.formatLine === "function") return M.formatLine(text);
    /* A part's NAME. A heading left in English over a Hebrew session was the one thing
       that still looked half-done (owner, 2026-09-20). Notes are still left alone — they
       are sentences, and a table cannot write one. */
    if (kind === "title" && typeof M.titleLine === "function") return M.titleLine(text);
    if (typeof M.hebrewOnly !== "function") return text;
    return M.hebrewOnly(text);
  }
    ...
    if (Array.isArray(declared) && declared.length) {
      for (i = 0; i < declared.length; i++) {
        ...
        out.push({
          index: out.length + 1,
          startWeek: sw,
          weekCount: Math.min(wc, total - sw + 1),
          /* The name he gave this month, if he gave it one. */
  ...
  function renderCalHtml(block, activeWi, activeDay, opts) {
    ...
    /* ── HOW MANY SESSIONS EACH WEEK HOLDS ──────────────────────────────────
       A client who trained four times a week and now trains five does not want his
       past月 rewritten: the change starts where he made it (owner, 2026-09-05). So a
       WEEK may say how many sessions it holds, and the programme's own number is only
       the default for the weeks that say nothing.

       The grid keeps ONE column count — the widest week — and a week with fewer
       sessions fills the rest of its row with empty places. That is what keeps week 3
       under week 2 instead of two grids of different widths side by side. */
    /* The programme's own number, kept apart from the widest week below — a week that
       says nothing must keep answering with the DEFAULT, not with the widest. */
    var baseCols = cols;
    function weekSessionsAt(wi) {
      var w = (block && block.weeks && block.weeks[wi]) || null;
      var n = parseInt(w && w.sessions, 10);
      if (n >= 1 && n <= 7) return n;
      return baseCols;
    }
    /* The grid is as wide as the WIDEST WEEK — and the programme's own number only
       counts through the weeks that are silent about theirs. Starting from the sale
       kept a five-column grid open after he had cut every week to three, so three
       sessions were drawn under five headings (owner, 2026-09-08). */
    var widest = 0;
    var wkList = (block && block.weeks) || [];
    for (var wsI = 0; wsI < wkList.length; wsI++) {
      var wsN = weekSessionsAt(wsI);
      if (wsN > widest) widest = wsN;
    }
    if (!widest) widest = cols;
    cols = widest;

    /**
     * How wide ONE block is drawn.
     *
     * Each month is its own grid with its own row of headings, so a block of three
     * sessions a week planted inside a programme of five is drawn as three — not as
     * three under five headings, which is what one width for the whole plan gave him
     * (owner, 2026-09-08).
     *
     * @param {{startWeek:number, weekCount:number}} g
     */
    function colsForGroup(g) {
      var from = Math.max(1, (g && g.startWeek) || 1) - 1;
      var count = Math.max(0, (g && g.weekCount) || 0);
      var out = 0;
      for (var i = from; i < from + count; i++) {
        var n = weekSessionsAt(i);
        if (n > out) out = n;
      }
      return out || cols;
    }
    ...
    var totalWeeks =
      opts.weekRows > 0 ? (opts.weekRows | 0) : Math.max(4, (block.weeks && block.weeks.length) || 0);
    ...
    /* The week he is LOOKING at, not the widest one in the plan. Since a week can hold
       its own number of sessions, the two are not the same — and a header that says 5
       over a week of 4 is a header that lies (owner, 2026-09-08). */
    if (bySession) spanLabel = weekSessionsAt(activeWi) + " sessions a week";
    var html =
    ...
    function cellHtml(weekIndex0, dayIndex, iso) {
      ...
      if (has) cls += " has-wod";
      /* Written, kept, and not being sent. The owner's screens draw it muted; the
         client's never receives it at all, so this class can only ever appear on his
         side (owner, 2026-09-16). */
      if (dayData.hidden === true) cls += " is-hidden";
    ...
    /**
     * The row of headings above a grid.
     * @param {number} [howMany] the columns of THIS grid; the whole plan's if omitted
     */
    function dowRowHtml(howMany) {
      var n = howMany >= 1 ? howMany : cols;
      var style = bySession ? ' style="--cal-cols:' + n + '"' : "";
      var row = '<div class="pprog-cal-dow"' + style + "><span></span>";
      var d;
      /* A session has no weekday, so its column is numbered rather than named. */
      if (bySession) {
        for (d = 0; d < n; d++) row += "<span>" + (d + 1) + "</span>";
        return row + "</div>";
      }
      for (d = 0; d < 7; d++) {
        if (dowSelectFn) {
    ...
    if (calMode === "month" || calMode === "block") {
      ...
      for (var gi = 0; gi < shown.length; gi++) {
        var g = shown[gi];
        var gCols = colsForGroup(g);
        var gStyle = bySession ? ' style="--cal-cols:' + gCols + '"' : "";
        html +=
          /* The block says which block it is, so the page can copy it, duplicate it,
             plant one in front of it or drag it (owner, 2026-09-05). */
          '<div class="pprog-cal-block" data-block="' + g.index + '">' +
          '<div class="pprog-cal-block-title" data-block-title="' + g.index + '">Block ' +
          g.index + " · " + esc(groupLabel(g)) +
          /* A name he gave the month reads after its dates, in his own language
             (owner, 2026-09-08). */
          (g.name ? '<span class="pprog-cal-block-name">' + esc(g.name) + "</span>" : "") +
          /* And where the page can save one, the pencil that asks for it. */
          (opts.allowBlockRename === true
            ? '<button type="button" class="pprog-block-rename" data-block-rename="' + g.index +
              '" title="שם ללבנה" aria-label="שם ללבנה">' +
        ...
        for (var wi = g.startWeek - 1; wi < g.startWeek - 1 + g.weekCount; wi++) {
          ...
          html += '<div class="pprog-cal-row' + (rowDeload ? " deload-week" : "") + '"' + gStyle + ">" + weekColBtn(wi);
          var rowCols = weekSessionsAt(wi);
          for (var di = 0; di < rowCols; di++) {
            html += cellHtml(wi, di, addDaysIso(start, wi * 7 + di));
          }
          /* A week that holds fewer sessions than the widest one IN ITS OWN BLOCK keeps
             its place in the grid rather than stretching to fill it. */
          for (var dz = rowCols; dz < gCols; dz++) {
            html += '<span class="pprog-cal-cell is-off" aria-hidden="true"></span>';
          }
          html += "</div>";
        }
        html += "</div></div>";
      ...
      html +=
        ...
        (bySession
          ? weekSessionsAt(activeWi) + " sessions a week · " + totalWeeks + " weeks · tap a session"
      ...
      html +=
        ...
        "</div>";
      var weekCols = weekSessionsAt(activeWi);
      var weekStyle = bySession ? ' style="--cal-cols:' + weekCols + '"' : "";
      html += dowRowHtml(weekCols);
      html += '<div class="pprog-cal-row"' + weekStyle + ">" + generalBtn(activeWi);
      for (var di2 = 0; di2 < weekCols; di2++) {
  ...
  /**
   * THE ENGLISH THE EDITOR CARRIES BUT NEVER SHOWS.
   *
   * A Hebrew programme keeps its English out of sight in `linesEn` / `titleEn`, because
   * that is the only language the mechanical check can read (lib/coach-brick-check.js).
   * The editor works in the language on screen — so without this, opening a day and
   * saving it handed the check a Hebrew line it cannot parse, and a whole session went
   * unchecked. It is exactly the failure that reported a week of "22 כפיפות בטן" as
   * having no bodyweight work in it (owner, 2026-09-20).
   *
   * Carried BY TEXT, not by position: the editor is free to reorder lines, drop an empty
   * one, or move a note, and each line still finds its own English. A line he EDITS finds
   * nothing — which is the point. Old English describing a movement he has just replaced
   * is worse than no English at all, so the changed line goes to the check as he typed it.
   */
  function englishBaggage(part) {
    var lines = (part && part.lines) || [];
    var en = (part && part.linesEn) || [];
    if (!Array.isArray(en) || !en.length) {
      if (!part || !part.titleEn) return null;
      en = [];
    }
    var byText = {};
    for (var i = 0; i < lines.length; i++) {
      var he = String(lines[i] == null ? "" : lines[i]).trim();
      var eng = String(en[i] == null ? "" : en[i]).trim();
      if (he && eng) byText[he] = eng;
    }
    return {
      byText: byText,
      title: String((part && part.titleEn) || ""),
      titleHe: String((part && part.title) || "").trim(),
    };
  }

  /** The English for one shown line: its own, or the line itself once he has edited it. */
  function englishFor(baggage, line) {
    var he = String(line == null ? "" : line).trim();
    var kept = baggage && baggage.byText ? baggage.byText[he] : "";
    return kept || he;
  }
  ...
  function draftFromDayData(dayData, restDay) {
    ...
    for (i = 0; i < parts.length; i++) {
      ...
      out.push({
        id: p.id || "",
        /* The NAME, without the heading's number: what he types here is the whole
           name of the part, and the number is drawn beside the field (owner,
           2026-09-09). */
        ...
        /* Not shown, not editable — see englishBaggage. */
        en: englishBaggage(p),
      });
    }
    return out;
  ...
  function partsFromDraft(draft) {
    ...
    for (var i = 0; i < src.length; i++) {
      ...
      var partTitle = String(p.title == null ? "" : p.title).trim();
      /* A part with nothing in it and nothing but its generated heading is not a part:
         it is the empty box the editor opens with. Saving it left an empty "Part A" in
         front of whatever he pasted underneath, and the day's focus line was then taken
         from THAT — which is how a finished session ended up drawn as a plan waiting to
         be filled (owner, 2026-09-08). */
      if (!lines.length && (!partTitle || isGeneratedPartTitle(partTitle))) continue;
      var entry = {
      ...
      /* The hidden English, rebuilt line for line beside what is shown. Only for a part
         that ARRIVED with English: a programme written in English has nothing to hide,
         and giving it a `linesEn` copy of itself would be noise in every stored block. */
      if (p.en) {
        entry.linesEn = lines.map(function (line) {
          return englishFor(p.en, line);
        });
        var titleEn = p.en.title && partTitle === p.en.titleHe ? p.en.title : "";
   ...
   */
  function colourPaletteHtml(setColourFn, pi, li, current, keyArg) {
  ...
  function renderAdminDayEditHtml(opts, restVisual) {
    ...
    /* The whole part, gone — the same × every other row in this editor has. */
    var removePartFn = hook(opts, "editRemovePart", "adminPprogEditRemovePart");
    ...
    /* WHICH DAY this editor belongs to.
       Several days can be open for editing at the same time (owner, 2026-09-09: "מצב
       עריכה מקבילה"), so every control has to name its own day — otherwise a button in
       one card edits whichever draft the page happened to open last. The key travels as
       the first argument of every hook: "3:tue".
       Empty for the older admin editor, which has exactly one draft and calls its hooks
       without a key — its call shape is therefore unchanged. */
    var editKey = String(opts.editKey || "").replace(/[^0-9a-zA-Z:_-]/g, "");
    var K = editKey ? "'" + editKey + "'," : "";
    var html = "";
    var pi;
    if (restVisual) {
      html +=
    ...
    for (pi = 0; pi < parts.length; pi++) {
      ...
      html += '<div class="section">';
      html += '<div class="section-title-row" data-part="' + pi + '">';
      /* Something to TAKE THE PART BY.
         Press-and-drag on the row itself only selected the words: most of that row is
         the title field, and a field is for typing in (owner, 2026-09-08). A grip says
         "this is the handle" and leaves the typing alone. Only where the page can
         actually move a part. */
      if (canPaint) {
        html +=
          '<span class="pprog-part-grip" data-part-grip="' + pi + '"' +
          ' title="גרור למעלה או למטה בתוך היום, או ליום אחר שפתוח על המסך" aria-hidden="true">⠿</span>';
      }
      /* The part's number, beside its name and not inside it: the field holds the
         name alone, so clearing the field clears the whole name (owner, 2026-09-09).
         It counts from the top and follows the part when it is dragged. */
      html += '<span class="pprog-part-n" aria-hidden="true">' + (pi + 1) + "</span>";
      html +=
        '<input class="pprog-edit-title section-title" type="text" value="' +
        esc(cleanPartTitle(part.title)) +
        '" placeholder="שם החלק" aria-label="Part title" oninput="' +
        setFn +
        "(" +
        K +
        pi +
      ...
      /* "Add numbering", beside the part's own heading and only while the day is open
         ...
         It is drawn as a PILL rather than a bare tick box: this row is the top of a
         session card, and a raw checkbox in the corner of it looked like a form
         (owner, 2026-09-08). The input is still a real checkbox, hidden behind it. */
      if (canPaint) html +=
        ...
        pi +
        ',this.checked)">' +
        "<span>Add numbering</span></label>";
      /* And the × that takes the whole part away, in the same shape as every other ×
         in this editor (owner, 2026-09-08). */
      html +=
        '<button type="button" class="pprog-edit-del-line pprog-del-part" onclick="' +
        removePartFn +
        "(" +
        K +
        pi +
        ')" title="מחק את החלק כולו" aria-label="מחק חלק">×</button>';
      html += "</div>";
      var ni;
      for (ni = 0; ni < notes.length; ni++) {
        html +=
          ...
          ni +
          ',this.value)"' +
          /* Enter opens another note right below this one, exactly as it does in a
             work line — notes come in twos and threes and reaching for the chip
             between them is the part that slows him down (owner, 2026-09-09). */
        ...
        html +=
          ...
          (part.numbered === true
            ? '<input type="text" class="pprog-li-num pprog-edit-num pprog-num-in" inputmode="numeric"' +
              ...
              ' oninput="' + setNumberFn + "(" + K + pi + "," + li + ',this)">'
            : "") +
          ...
          (canPaint
            ? '<button type="button" class="pprog-colour-btn"' +
              ...
              '<path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg></button>' +
              colourPaletteHtml(setColourFn, pi, li, partColours[li], K)
            : "") +
          '<button type="button" class="pprog-edit-del-line" onclick="' +
          removeWorkFn +
          "(" +
    ...
    html +=
      ...
      cancelFn +
      "(" + (K ? K.slice(0, -1) : "") + ')">Cancel</button>' +
      '<button type="button" class="pprog-edit-save" onclick="' +
      saveFn +
      "(" + (K ? K.slice(0, -1) : "") + ')">Save</button>' +
      "</div>";
    return html;
  }
  ...
  function renderDayCardHtml(block, week, activeWi, day, opts) {
    ...
    /* A FUNCTION, not a string, because several cards are on screen at once when he
       compares the same session across four weeks — one string meant every card wore
       the label of whichever day was selected last, so four cards all read
       "אימון 1 · שבוע 4" (owner, 2026-09-09, with the picture). A string is still
       accepted: the athlete app passes one and has a single card. */
    var overrideLabel =
      typeof opts.dateLabelOverride === "function"
        ? opts.dateLabelOverride(activeWi, day)
        : opts.dateLabelOverride;
    var dateLabel = overrideLabel
    ...
    html += '<div class="pprog-day-card' +
      ...
      /* Muted on his screen, absent from theirs (owner, 2026-09-16). */
      (dayData && dayData.hidden === true ? " is-hidden" : "") +
      '" dir="ltr" data-wi="' +
      (activeWi | 0) +
      '" data-day="' +
      day +
    ...
    var titleEditor =
      editing && opts.allowTitleEdit === true
        ? '<input type="text" class="pprog-day-title-in" data-day-title="' +
          esc(String(opts.editKey || "1")) + '" maxlength="80" dir="auto"' +
    ...
    /* The page's own controls in the card head — the rest tick. A FUNCTION when
       several days can be open at once, so each card gets its own (owner, 2026-09-09);
       a plain string still works for a page with one open day. */
    var editHeaderAction = editing
      ? typeof opts.editHeaderActionsHtml === "function"
        ? String(opts.editHeaderActionsHtml(activeWi, day) || "")
        : String(opts.editHeaderActionsHtml || "")
      : "";
    ...
    if (restDay) {
      ...
      /* The stub — "Planned focus · Full session details still loading…" — belongs to a
         day the coach's brain has planned but not yet filled. A day whose parts he
         NAMED himself is not that day, and replacing his headings with a loading
         message read as if his work had been lost (owner, 2026-09-08). */
      var anythingWritten = parts.some(function (p) {
        if (!p) return false;
        if (p.lines && p.lines.length) return true;
        var t = String(p.title || "").trim();
        return !!t && !isGeneratedPartTitle(t);
      });
      if (!anythingWritten && focus && focus !== "—") {
  ...
  /**
   * The draft for one day, from either shape the page may pass.
   *
   * `editDrafts` is a map keyed "wi:day" — several days open at once. `editDraft` is
   * the older single draft, still what the athlete app and the older admin editor
   * pass, and still exactly as it was.
   */
  function draftFromOpts(opts, wi, day) {
    var many = opts && opts.editDrafts;
    if (many && typeof many === "object") {
      var found = many[(wi | 0) + ":" + String(day || "")];
      return found || null;
    }
    var one = opts && opts.editDraft;
    if (!one || !opts.editing) return null;
    return (one.wi | 0) === (wi | 0) && String(one.day || "") === String(day || "") ? one : null;
  }
  ...
  function renderBrickView(opts) {
    ...
    if (selectedDays.length >= 2) {
      ...
      for (si = 0; si < selectedDays.length; si++) {
        ...
        /* Never a slot that was not sold. A selection is arithmetic on a 7-day grid, so
           a drag across a block used to ask for cards that do not exist — drawn as rest
           days with dates nobody chose (owner, 2026-09-09). The page filters its own
           selection; this is the card refusing to draw one anyway. */
        if (opts.sessionColumns && DAY_KEYS.indexOf(sday) >= (opts.sessionColumns | 0)) continue;
        var per = Object.assign({}, opts);
        var mine = draftFromOpts(opts, swi, sday);
        per.editing = !!mine;
        per.editDraft = mine;
        /* A key only where the page asked for several open days. A page passing the
           single `editDraft` keeps the old call shape, argument for argument. */
        per.editKey = mine && opts.editDrafts ? swi + ":" + sday : "";
      ...
      var one = Object.assign({}, opts);
      var oneDraft = draftFromOpts(opts, wi, day);
      if (opts.editDrafts) {
        one.editing = !!oneDraft;
        one.editDraft = oneDraft;
      }
      if (opts.editDrafts && one.editDraft) one.editKey = wi + ":" + day;
      card = (opts.widthStatusHtml || "") + renderDayCardHtml(block, week, wi, day, one);
    } else {
      card =
        (opts.widthStatusHtml || "") +
  ...
  function dayShareText(block, weekIndex0, dayKey, opts) {
    ...
    for (var pi = 0; pi < parts.length; pi++) {
      ...
      t += formatPartHeading(pi, part.title) + "\n";
  ...
  return {
    ...
    esc: esc,
    /* Hebrew beside the English, set once by the page from the programme's own field. */
    ...
    englishBaggage: englishBaggage,
    englishFor: englishFor,
    isGeneratedPartTitle: isGeneratedPartTitle,
    cleanPartTitle: cleanPartTitle,
