    ...
    if (calMode === "month" || calMode === "block") {
      ...
      for (var gi = 0; gi < shown.length; gi++) {
        ...
        for (var wi = g.startWeek - 1; wi < g.startWeek - 1 + g.weekCount; wi++) {
          ...
          /* A week that holds fewer sessions than the widest one keeps its place in the
             grid rather than stretching to fill it. */
          for (var dz = rowCols; dz < cols; dz++) {
      ...
      for (var dz2 = weekCols; dz2 < cols; dz2++) {
        html += '<span class="pprog-cal-cell is-off" aria-hidden="true"></span>';
      }
      html += "</div>";
      html +=
  ...
  function renderAdminDayEditHtml(opts, restVisual) {
    ...
    for (pi = 0; pi < parts.length; pi++) {
      ...
      if (canPaint) html +=
        ...
        "(" +
        pi +
        ',this.checked)"><span class="pprog-num-pill">' +
        '<span class="pprog-num-mark" aria-hidden="true">1</span>' +
        "<span>Add numbering</span></span></label>";
