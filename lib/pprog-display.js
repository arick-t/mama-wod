...
(function (root, factory) {
  ...
  function renderCalHtml(block, activeWi, activeDay, opts) {
    ...
    if (bySession) spanLabel = cols + " אימונים בשבוע";
    var html =
    ...
    function dowRowHtml() {
      var row = '<div class="pprog-cal-dow"><span></span>';
      var d;
    ...
    if (calMode === "month") {
      ...
      /* The floor of 5 exists for the ATHLETE app: a brick is always five weeks, and a
         block that is still loading must not collapse the calendar to two rows. A client
         program can genuinely BE four weeks (no deload), and showing it a W5 rail that
         leads nowhere is wrong — so the caller may state the real count. */
      var weekCount = opts.weekRows > 0
        ? (opts.weekRows | 0)
        : Math.max(5, (block.weeks && block.weeks.length) || 0);
      ...
      html +=
        ...
        (bySession
          ? cols + " sessions a week · " + weekCount + " weeks · tap a session"
          : "Full " + weekCount + "-week brick · tap a date · ‹ › moves active week") +
        "</div>";
    } else {
      var activeStart =
  ...
  function draftFromDayData(dayData, restDay) {
    ...
    for (i = 0; i < parts.length; i++) {
      ...
      var c = classifyPartLines(p.lines) || { notes: [], format: "", work: [], trailingNotes: [] };
  ...
  function renderAdminDayEditHtml(opts, restVisual) {
    ...
    for (pi = 0; pi < parts.length; pi++) {
      ...
      for (ni = 0; ni < notes.length; ni++) {
        html +=
          '<input class="pprog-edit-note pprog-part-note" type="text" value="' +
          esc(notes[ni] || "") +
      ...
      html +=
        '<input class="pprog-edit-format pprog-part-format" type="text" value="' +
        esc(part.format || "") +
        ...
        pi +
        ",'format',this.value)\">";
      html += '<ul class="section-lines pprog-edit-lines">';
      var li;
      for (li = 0; li < work.length; li++) {
        html +=
          '<li class="pprog-edit-work-row"><input type="text" value="' +
          esc(work[li] || "") +
          ...
          ",'work'," +
          li +
          ',this.value)"><button type="button" class="pprog-edit-del-line" onclick="' +
          removeWorkFn +
          "(" +
          pi +
          "," +
  ...
  function dayShareText(block, weekIndex0, dayKey, opts) {
    ...
    for (var pi = 0; pi < parts.length; pi++) {
      var part = parts[pi] || {};
      var c = classifyPartLines(part.lines) || { notes: [], format: "", work: [], trailingNotes: [] };
