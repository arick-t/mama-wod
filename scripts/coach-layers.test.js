...
function testRouterAgainstRealPacket() {
  ...
  /* Corrected on the merge (2026-09-03): the admin release IS the local builder now, so
     today's packet DOES carry a COMPETITOR line. The legacy case has to be built by
     removing the line, not by hoping the builder omits it — otherwise this asserted the
     opposite of the rule above, that an explicit line is authoritative. */
  const legacyPacket = function (over) {
    const p = withPacket(over);
    return {
      fixedIntakePacket: String(p.fixedIntakePacket).replace(/^COMPETITOR:.*$/gm, ""),
      goals: p.goals,
      injuries: p.injuries,
    };
  };
  ok("a packet with no COMPETITOR line still falls back to stated intent",
    L.competitorDeclared(legacyPacket({ goals: "I want to compete at a local throwdown" }), null) &&
      !L.competitorDeclared(legacyPacket(), null));
